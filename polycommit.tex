...
Lemma \ref{lem:forking} in each invocation guarantees that the extracted parent polynomial has coefficients at most $(p-1)$ larger than the coefficients of the childrens polynomials. Given that the transcript tree has depth $\lceil \log_2(d+1)\rceil$ we get that the final extracted polynomial $f^{(0)}(X)\in (p-1)^{\lceil \log_2(d+1)\rceil} \frac{p-1}{2}(\frac{p+1}{2})^{\lceil \log_2(d+1)\rceil}$ 
