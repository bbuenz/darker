...
\paragraph{Notation}
\begin{itemize}
\item Let $f(X) \in \mathbb{F}_p[X]$ be a polynomial of degree at most $d$. The coefficients of $f(X)$ are denoted by $f_i$ such that $f(X) \stackrel{\triangle}{=} \sum_{i=0}^{d} f_i X^i$.
...
\end{lemma}
\begin{proof}
	Given an adversary $\adv_{\textsf{FractionalRoot}}$ that succeeds in breaking the fractional root assumption for $\ggen$ we can construct either an adversary $\adv_{RSA}$ for the strong RSA assumption or an adversary $\adv_{\textsf{Order}}$ that breaks the order assumption for $\ggen$. As shown in Lemma \ref{lem:ordertoadaptive} the order assumption reduces to the adaptive root assumption with overwhelming probability. 
	...
	We now run the $\adv_{\textsf{FractionalRoot}}$ on input $\GG$ and $\gr{g}$ to generate a tuple $(\alpha,\beta,\gr{u})$ such that $\gr{u}^{\beta}=\gr{g}^{\alpha}$. Let $\gamma=\gcd(\alpha,\beta)$ and $\alpha'=\frac{\alpha}{\gamma}\in \ZZ$ and  $\beta'=\frac{\beta}{\gamma}$. Now either $\gr{g}^{\alpha'}=\gr{u}^{\beta'}$ or $\gr{g}^{\alpha'}/\gr{u}^{\beta'}$ is a non trivial element of order $\gamma$ which would directly break the order assumption. In that case we constructed $\adv_{\textsf{Order}}$ that outputs $(\gr{g}^{\alpha'}/\gr{u}^{\beta'},\gamma)$.
	
	Now assume otherwise, i.e. $\gr{g}^{\alpha'}=\gr{u}^{\beta'}$. By construction $\gcd(\alpha',\beta')=1$ and we can efficiently compute integers $a,b$ such that $a \alpha'+b \beta'=1$. By assumption on $\adv_{\textsf{FractionalRoot}}$ $\beta'$ is not $1$. Now let $\gr{w}\gets \gr{u}^{a}\gr{g}^{b}$. Note that $\gr{w}^{\alpha'\beta'}=\gr{g}^{\alpha'}$. So either $\gr{w}^{\beta'}=\gr{g}$ or $\gr{w}^{\beta'}/\gr{g}$ is a non-trivial element of order $\alpha'$. The first case breaks the strong RSA assumption, as we can construct $\adv_{\textsf{RSA}}$ that outputs $(\gr{w},\beta)$, and the second breaks the order assumption.
\end{proof}
...
We consider several candidate groups of unknown order. First the RSA group which is the multiplicative group $\ZZ_n$ for a composite integer $n=p\cdot q$ for primes $p$ and $q$. Computing the order of 
$\ZZ_n$ is as hard as factoring $n$. Unfortunately, the order assumption does not hold for $\ZZ_n$ this is because $-1 \in \ZZ_n$ can be easily computed and has order two. This can be resolved though by working in the quotient group $\ZZ_n\{-1,1\}$ which does not contain an element of order $2$. Alternatively one can use the subgroup of quadratic residues $QR\subset \ZZ_n$ which also does not contain elements of known order. Additionally if $n$ is the product of strong primes, i.e. $\frac{p-1}{2}$ and $\frac{q-1}{2}$ are primes, then $QR\subset\ZZ_n$ does not contain elements of low order\cite{JC:FisSch00,C:HofKil09}. In this group the order assumption and the strong RSA assumption are equivalent to the hardness of factoring $n$.

Generating $n$ requires a private coin trusted setup as the factorization of $n$ needs to be unknown. To circumvent this we can use the class groups of imaginary quadratic order \cite{Fill in} which are candidate groups of unknown order but do not require a trusted setup. Specifically, we can define a class group $CL(p)$ by sampling a prime discriminant $p$. $p$ can be public which means that the setup simply requires random bits for sampling $p$. If $p=7 \bmod 8$ then we know that the order of $p$ is odd and that no elements of order $2$ exist.
Working in $CL(p)$ presents us with several difficulties. Most importantly, there exists an efficient polynomial time algorithm due to Gauß and described by Bosma and Stevenhagen\cite{bosma1996computation} to compute square roots of arbitrary elements in $CL(p)$. This algorithm can be leveraged to compute arbitrary power of two roots. 
Class groups can therefore not be used to cryptographically commit to integers but rather to rationals, whose denominator is a power of $2$. This set of rationals is called the \emph{dyadic rationals} and is denoted by 
${\mathcal{D}\subset \QQ=\{d\in \QQ\; |\; \exists k \in \NN \text { s.t } 2^k \cdot d\in \ZZ\}}$. 
Additionally the strong RSA assumption and as such the fractional RSA assumption are both broken if computing square roots is easy. We, therefore, now state modified assumptions which are believed to still hold even if computing square roots is easy.
...
\begin{minipage}{\textwidth}
        ...
        \item \pcind[1] \verifier checks that $b< \mu_{p,d}\leq q$\pccomment{$\mu_{p,d}=O(p^{\log(d)})$ is a constant}
...
\begin{proof}
	...
	\benedikt{This seems long for the correctness proofs. I don't really like the term activate.}
\end{proof} 
...
\end{lemma}
\begin{proof}
We show that if an extractor $E'$, as defined in Definition~\ref{def:wee}, exists for the protocol $\eval'$ then we can construct an extractor $E$ for the protocol $\eval$. 
For each PPT adversary $(\adv,P^*)$, $E$ will receive a polynomial number of transcripts. Any transcript $\tr$ of $\eval$ such that $\adv(\tr)=1$ and $\tr$ is accepting contains exactly $\lceil \log(d+1)\rceil$ $\textsf{PoE}s$ executions. 
In total $E$ sees a polynomial number of $\textsf{PoE}$ executions. By Lemma~\ref{lem:poe} under the adaptive root assumption the probability that a polynomial time adversary can break the soundness of $\textsf{PoE}$, i.e. convince a verifier on a $\textsf{PoE}$ instance $(\gr{C}_R,\gr{C}/\gr{C}_{L},q^{d'+1})$, where $(\gr{C}_R,\gr{C}/\gr{C}_{L},q^{d'+1})\not\in\mathcal{R}_{\textsf{PoE}}$, is negligible. 
Consequently the probability that the adversary can break $\textsf{PoE}$ on \emph{any} of the polynomial number of executions is still negligible.

This means that with overwhelming probability all transcripts are equivalent to having the verifier directly check $(\gr{C}_R,\gr{C}/\gr{C}_{L},q^{d'+1})\in\mathcal{R}_{\textsf{PoE}}$. Now we forward all these transcripts, without the $\textsf{PoE}$ to the $\eval'$ extractor $E'$. By assumption, $E'$ succeeds with overwhelming probability in extraction on all accepting transcripts $\tr$ such that $\adv(\tr)=1$. $E$ can simply output the same witnesses as $E'$. $E$ will therefore be a valid extractor for the protocol $\eval$ which shows that under the adaptive root assumption $\eval$ has witness extended emulation and by definition so does the polynomial commitment scheme.
\end{proof}
...
The $\eval$ protocol combines two statements into one by using a random linear combination of group elements, i.e. $\gr{C}'\gets \gr{C}_L^{\alpha}\gr{C}_R$. We now show that this step is sound and that given the discrete logarithm for $\gr{C}'$ we can extract the discrete logarithm for $\gr{C}_L$ and $\gr{C}_R$ we also show that the we can bound the size of the discrete logarithm. We show that this statement holds in two settings. First we consider a group $\GG$ were the standard strong RSA assumption holds and group elements are encodings of integers. Then we will show that in groups were taking square roots is easy we can extract dyadic rationals using the dyadic strong RSA assumption.
 %For $\GG \sample \ggen(\lambda)$, and $\gr{g}\sample \GG$. Let $(\gr{C}_L\in \GG,\gr{C}_R\in \GG,\alpha\in [0,p-1],f\in [0,b];\gr{g}^{f}=\gr{C}_L^{\alpha}\gr{C}_R)$ and  $(\gr{C}_L,\gr{C}_R,\alpha'\in [0,p-1],f'\in [0,b];\gr{g}^{f'}=\gr{C}_L^{\alpha'}\gr{C}_R)$  be two transcripts for $\alpha\neq \alpha'$.
\begin{lemma}
	For $\GG \sample \ggen(\lambda)$, and $\gr{g}\sample \GG$. Let $(\gr{C}_L\in \GG,\gr{C}_R\in \GG,\alpha\in [0,p-1],|f|\leq b;\gr{g}^{f}=\gr{C}_L^{\alpha}\gr{C}_R)$ and  $(\gr{C}_L,\gr{C}_R,\alpha'\in [0,p-1],f'\in [0,b];\gr{g}^{f'}=\gr{C}_L^{\alpha'}\gr{C}_R)$ be two transcripts for $\alpha\neq \alpha'$. There exists a PPT algorithm $\mathcal{X}$ that given these transcripts can compute either (1) $f_1,f_2\in \ZZ$ such that $\gr{g}^{f_1}=\gr{C}_1$, $\gr{g}^{f_1}=\gr{C}_2$, $|f_1|\leq b \cdot p$ and $|f_2|\leq b \cdot p$ or (2) an element in $\GG$ of known order or a non-trivial root of $\gr{g}$.
\end{lemma}
\begin{proof}
	Using the transcripts we compute $\gr{C}_{L}^{\alpha-\alpha'}=\gr{g}^{f-f'}$. 
 If $\frac{f-f'}{\alpha-\alpha'}$ is not an integer then this gives us a fractional root of $\gr{g}$, that is the tuple $(f-f',\alpha-\alpha',\gr{C}_{L})$. By Lemma~\ref{lem:strongtofractional}, from a fractional root we can either compute an element of known order or the $\frac{\alpha-\alpha'}{gcd(f-f',\alpha-\alpha')}\in \ZZ$'s root of $\gr{g}$.  
 If $\frac{f-f'}{\alpha-\alpha'}$ is indeed an integer then we can compute $\gr{D}=\gr{g}^{\frac{f-f'}{\alpha-\alpha'}}$. Either $\gr{D}=\gr{C}_{L}$ or $(\gr{D}/\gr{C}_{L})^{\alpha-\alpha'}=1$, \emph{i.e.} $\gr{D}/\gr{C}_{L}$ is an element of known order. 
  
Otherwise we have $\gr{C}_{L}=\gr{g}^{f_L}$ where $f_L=\frac{f-f'}{\alpha-\alpha'}$ is an integer.
Additionally $\gr{C}_R=\gr{g}^{f_R}$ for $f_R=f-\alpha f_L$.  
\benedikt{Finish proof by computing order}
\end{proof}
\begin{theorem}
