...
In our running example, $f_L(X)=4x+1$ and $f_R(X)=2x+3$. Note that the verifier can check the consistency of these commitments by testing $\gr{G}_L\gr{C}_R^{q^{d'+1}}=\gr{C}$. Now we reduce the problem to a single, smaller statement by taking a random linear combination between $\gr{C}_L$ and $\gr{C}_R$, \emph{i.e.}, $\gr{C}'\gets \gr{C}_L^{\alpha}\gr{C}_R=\gr{g}^{(\alpha f_L+f_R)(q)}$. Now we recurse on $\gr{C}'$ with the statement that it commits to a degree $d'$ polynomial. 
