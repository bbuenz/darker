...
As explained in Section \ref{subsec:optimization} the commitment scheme has several batching properties. It is possible to evaluate $k$ polynomials of degree at most $d$ using only $2$ group elements and $(k+1)$ field elements. To take advantage of this we delay the evaluation until the last step of the protocol (see Section \ref{subsec:compiling}). We present the proof size for both the compilation of Sonic and Plonk. We use 1200 bits as the size of the class group and $\lambda=120$. This leads to proof sizes of $11KB$ for Sonic and $7.8$KB using Plonk for circuits with a million gates.
\begin{mdframed}
...
\hline
 ...
 & & & $+(27+9\log_2(n))\ZZ_p$ &  \\
  Plonk\cite{PLONK}& $4$ in $\crs$ + 6 & $2$& $6+2\log_2(3n)\GG$& 7.8 KB\\
  & & & + $(10+3\log_2(3n))$
	\end{tabular}
\end{mdframed}
