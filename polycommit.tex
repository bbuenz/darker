...
$\mathcal{X}$ given a tree of $\eval'$ transcripts, recursively either extracts the encoding of an integer polynomial $f(X)\in \mathbb{D}[X]$ with bounded coefficients or a break of the Order Assumption or the Fractional Root Assumption. 
...
Now if $d_i+1$ is even then we can use Lemma~\ref{lem:intrandomcombine} to extract two bounded polynomials $f_{L}^{(i)},f_{R}^{(i)}$.

 have for a challenge $\alpha$: $\gr{C}_{L,i-1}^\alpha \gr{C}_{R,i-1}=\gr{C}_i=\gr{g}^{f^{(i)}(q)}$ additionally for a challenge $\alpha'\neq \alpha$ we have $\gr{C}_{L,i-1}^{\alpha'} \gr{C}_{R,i-1}=\gr{C}'_i=\gr{g}^{f'^{(i)}(q)}$.  
