...
\begin{itemize}
    \item[] \textbf{Hard problem.}\alan{This item  is no longer valid.} The soundness or knowledge-soundness of our proof systems reduces the hardness of a new hard problem called the \emph{fractional root problem} which informally states that a computationally bounded adversary cannot produce two group elements $g, h \in \mathbb{G}$ with non-trivial exponents $a, b \in \mathbb{Z}$ such that $g^a = h^b$. We prove the hardness of this problem in the generic group model and show that it generalizes both the strong RSA problem and the adaptive root problem\alan{TODO: verify}. Given the new-found popularity of groups of unknown order, which provide the context for this hard problem, and given the genericity of this hard problem,  it is important to establish the correct definition and justify the matching hardness assumption as an independent contribution.
...
\item $p$ is a prime. $\primes$ denotes the set of primes less than $2^{\lambda \log(\lambda)}$. There are at least $2^\lambda$ primes in that set.
...
The cryptographic compilers make extensive use of groups of unknown order, i.e. groups for which the order cannot efficiently be computed.
...
Both of these assumption hold in generic groups of unknown order\cite{genericunknown,C:BonBunFis19}, i.e. there are no efficient algorithms that only have blackbox access to the group but are able to break these assumptions. 
...
\[
    \Pr\left[u^\ell = g \, \wedge \, \ell > 2
    :
    \begin{array}{l}
         \GG \leftarrow \ggen(\lambda)  \\
         g \sample \GG \\
         (u, \ell) \in \mathbb{G} \times \mathbb{N} \leftarrow \adv(\mathbb{G}, g) \\
    \end{array}\right] \leq \negl \enspace .
\]
\end{assumption}
...
First, $\adv_0$ outputs an element $w \in \GG$ and some $\state$.
Then, a random prime $\ell$ in $\primes$ is chosen
and $\adv_1(\ell,\state)$ outputs $w^{1/\ell} \in \GG$.
More precisely, for all efficient $(\adv_0,\adv_1)$:
\[           \advantage{AR}{(\adv_0,\adv_1)}\deq 
                \Pr\left[u^\ell = w \neq 1 \ : \ 
                \begin{array}{l}
                      \GG \sample \ggen(\lambda) \\ 
                      (w,\state) \sample \adv_0(\GG) \\
                      \ell \sample \primes \\ 
                      u \sample \adv_1(\ell, \state)
                \end{array} 
        \right] \leq \negl.
\]
\end{assumption}
...
\[        
                \Pr\left[w\neq 1 \wedge w^{\alpha}=1  : 
                \begin{array}{l} 
                      \GG \sample \ggen(\lambda) \\ 
                      (w,\alpha) \sample \adv(\GG) \\
...
\end{lemma}
\begin{proof}
	We show that given an adversary $\adv_{\textsf{Order}}$ that breaks the order assumption we can construct with overwhelming probability $\adv_{\textsf{Adaptive Root}}$ that breaks the adaptive root assumption. We run $\adv_{\textsf{Order}}$ to get a $v\neq 1\in \GG$ and $\alpha \in \ZZ$ such that $v^{\alpha}=1$. To construct $\adv_{\textsf{Adaptive Root}}$, $\adv_{\textsf{Adaptive Root},0}$ outputs $(v,\alpha)$. The challenger generates a random challenge $\ell$. If $\gcd(\ell,\alpha)=1$ then $\adv_{\textsf{Adaptive Root},1}$ can compute $\beta\gets \ell^{-1} \bmod \alpha$ and output $w^{\beta}$. The probability that $\gcd(\ell,\alpha)=1$ is overwhelming because $\gcd(\ell,\alpha)\neq 1 \implies \ell \not\vert \alpha$. This happens with negligible probability as $\ell$ is picked from a set of $2^\lambda$ primes and at most $\poly$ distinct primes can divide $\alpha$.
	\end{proof}
...
\[        
                \Pr\left[y^\beta = g^{\alpha} \wedge \vert\frac{\beta}{\gcd(\alpha,\beta)}|>2   : 
                \begin{array}{l} 
                      \GG \sample \ggen(\lambda) \\ 
                      g \sample \GG \\
                      (\alpha, \beta, y) \sample \adv(\GG, g) \\
                      \quad \textnormal{where} \, |\alpha|<2^{\poly}, |\beta|<2^{\poly} \in \ZZ \\
                      \quad \textnormal{and} \, y \in \GG 
                \end{array} 
        \right] \leq \negl \enspace .
\]
\end{assumption}
...
\begin{proof}
	...
	First we run $\adv_{RSA}$ to generate $g\sample \GG$.
	We run the $\adv_{\textsf{PseudoRoot}}$ on input $\GG$ and $g$ to generate a tuple $(\alpha,\beta,y)$ such that $y^{\beta}$ and $g^{\alpha}$. Let $\gamma=\gcd(\alpha,\beta)$ and $\alpha'=\frac{\alpha}{\gamma}\in \ZZ$ and  $\beta'=\frac{\beta}{\gamma}$. Now either $g^{\alpha'}=y^{\beta'}$ or $g^{\alpha'}/y^{\beta'}$ is a non trivial element of order $\gamma$ which would break the root assumption.
	By construction $\gcd(\alpha',\beta')=1$ and we can efficiently compute integers $a,b$ such that $a \alpha'+b \beta'=1$. By assumption $\beta'$ is not $1$. Now let $w\gets y^{a}g^{-b}$. Note that $w^{\alpha'\beta'}=g^{\alpha'}$. So either $w^{\beta'}=g$ or $w^{\beta'}/g$ is a non-trivial element of order $\alpha'$. The first case breaks the strong RSA assumption and the second breaks the order assumption.
\end{proof}
