...
We use subscripts to denote the inputs $\gr{C}_i$ and $y_i$ to $\pro{MultiEval}$ such that the subscript $i$ corresponds to the call of $pro{MultiEval}$ where $n=i$ and the witness polynomial has $i$ variables. 
For the witness polynomials $f^{(i)}(X_1,\dots,X_{i})$ we use superscripts instead of subscripts to distinguish it from the notation for coefficient.
	...
	If $n=1$ the $\mathcal{X}_{\pro{Eval'}}$ extracts a univariate degree $d$ polynomial $f^{(1)}(X_1)$ such that $f^{(1)}(z_1)=y_1$ or an element of known order in $\GG$ or a fractional root of $\gr{g}$. 
	If $n>1$ we can recursively extract. Given the $i-1$ variate degree polynomial we call the univariate extractor with $f=f^{(i-1)}(q_1,\dots,q_{i-1})$ as the final prover output. Note that $f<q_{i-1}\cdot b_{i-1}<q_{i}/2$ so the univariate extractor will extract a witness to an assumption break or a univariate polynomial with coefficients less than $b_i\cdot q_i$. We can use the decoding algorithm to decode each coefficient into a $i-1$ variate polynomial with coefficients bounded by $b_i$ such that we can compute $f^{(i)}(X_1,\dots,X_i)\in B_{b_i}$ an $i$-variate polynomial with bounded coefficients. 
