...
\begin{minipage}{\textwidth}
	\begin{flushleft}
	$\pro{ExtractCoefficient}(\params, \gr{C}, i; f(X)):$ \pccomment{$f(X) \in \ZZ[X]$}
		\begin{enumerate}[nolistsep]
		    \item \prover computes $f_L(X) \gets \sum_{j=0}^{i-1} f_j X^j$ and $f_R \gets \sum_{j=i+1}^d f_j X^{j-i-1}$
		    \item \prover computes $\gr{C}_L \gets \gr{g}^{f_L(q)}$, $\gr{C}_a \gets \gr{g}^{f_i}$, and $\gr{C}_R \gets \gr{g}^{f_R(q)}$
		    \item \prover computes $\gr{C}_a' \gets \gr{C}_a^{q^i}$ and $\gr{C}_R' \gets \gr{C}_R^{q^{i+1}}$
		    \item \prover sends $\gr{C}_L, \gr{C}_a, \gr{C}_a', \gr{C}_R$ to \verifier
...
\begin{minipage}{\textwidth}
	...
	$\pro{OpenIndex}(\params, \gr{C}, a, i; f(X)):$ \pccomment{$f(X) \in \ZZ_p[X]$}
		\begin{enumerate}[nolistsep]
		    \item \prover and \verifier run $(\gr{C}_a; f_i) \gets \pro{ExtractCoefficient}(\params, \gr{C}, i; f(X))$
		    \item \textbf{return} $\pro{Open}(\params, \gr{C}_a, a; f_i)$
		\end{enumerate}
	\end{flushleft}
\end{minipage}
\end{mdframed}
...
\begin{minipage}{\textwidth}
	\begin{flushleft}
	$\pro{InnerProduct}(\params, \gr{C}_\mathbf{f}, \gr{C}_a; \mathbf{f}, \mathbf{s}, \hat{\mathbf{f}}):$ \pccomment{$\mathbf{f}, \mathbf{s} \in \ZZ_p^{d+1}$ but $\hat{f}(X) \in \mathbb{Z}[X]$}
		\begin{enumerate}[nolistsep]
