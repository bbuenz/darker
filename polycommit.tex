...
Let $[\![ \cdot ]\!] : \mathbb{Z}_p[X] \rightarrow \mathbb{S}$ be a homomorphic commitment function that sends polynomials over a prime field to elements of some set $\mathbb{S}$. Moreover, let $\mathbb{S}$ be equipped with operations $\cdot + \cdot : \mathbb{S} \times \mathbb{S} \rightarrow \mathbb{S}$ and $\cdot \times \cdot : \mathbb{Z}_p[X] \times \mathbb{S} \rightarrow \mathbb{S}$ that accomodate two homomorphisms for $[\![ \cdot ]\!]$:
\begin{itemize}
    \item a \emph{linear homomorphism}: $a \times [\![f(X)]\!] + b \times [\![g(X)]\!] = [\![af(X) + bg(X)]\!]$; and
    \item a \emph{monomial homomorphism}: $X^d \times [\![f(X)]\!] = [\![X^d f(X)]\!]$.
\end{itemize}
For now, assume both prover and verifier have oracle access to the function $[\![\cdot]\!]$ and to the operations $ \cdot \times \cdot$ and $\cdot + \cdot$. (Later on, we will instantiate this commitment function using of unknown order and using the integer encoding of polynomials.)
...
The commitment function binds the prover to one particular polynomial for every commitment held by the verifier. In particular, at the start of every recursion step, the verifier is in possession of a commitment $[\![f(X)]\!]$ to $f(X)$. The prover provides commitments $[\![f_L(X)]\!]$ and $[\![f_R(X)]\!]$, and the verifier checks their soundness homomorphically via $[\![f(X)]\!] = [\![f_L(X)]\!] + X^{d'+1} \! \times \! [\![f_R(X)]\!]$. From these commitments, the verifier can also compute the commitment to $f'(X)$ homomorphically, via $[\![f'(X)]\!] = \alpha \! \times \! [\![f_L(X)]\!] + [\![f_R(X)]\!]$. In the last step, the verifier checks that the constant polynomial $f$ matches the commitment by computing $[\![f]\!]$ outright. These operations give rise to the following informal pseudocode description of the \eval protocol.
\begin{mdframed}
...
The last step in the presentation of the commitment scheme is to instantiate the abstract homomorphic hash function $[\![ \cdot ]\!]$. To this end we sample a group of unknown order $\mathbb{G}$, and sample a random element $\gr{g}$ from this group. Then $[\![z]\!]$ corresponds to raising $\gr{g}^z$, $\cdot + \cdot$ corresponds to the group operation, and $\cdot \times \cdot$ corresponds to exponentiation. All these operations can be computed locally. The $\setup, \pro{commit}$ and $\open$ functionalities are presented formally below. Note that the scheme is parameterized by $p$ and $q$; these values are determined by the context and independently of $\setup$.
...
Moving towards a formal presentation, it remains to specify some points we previously glossed over. First, if $d+1$ is not a power of 2, then we make a distinction between whether it is odd or even. If it is even, the recursion proceeds as normal for one step. If it is odd, then the polynomial is shifted by one digit --- specifically, $f'(X) = X f(X)$ and the protocol proceeds to prove that $f'(X)$ has degree bounded by $d' = d+1$ and evaluates to $y' = zy$ in $z$. The verifier obtains the matching commitment $\gr{C}'$ by raising $\gr{C}$ to the power $q$, or by outsourcing this task to the prover with a $\mathsf{PoE}$. Second, as the protocol proceeds the coefficients of $f(X)$ grow larger and larger, but eventually they have to be tested against some bound because coefficients that are \emph{too large} should be rejected. To take these growing coefficients into account we invoke a subroutine $\pro{EvalBounded}$, which takes an additional argument $b$ and which proves, in addition to what $\pro{Eval}$ proves, that all coefficients $f_i$ of $f(X)$ satisfy $|f_i| \leq b$. This subroutine is particularly useful if commitments were homomorphically combined prior to the execution of $\pro{EvalBounded}$. We now present the full, formal $\eval$ protocol below.
...
A commitment to $f \in \FF_p$ is then $\gr{g}^{\bar{f}(q)} \gr{h}^r$ for random $r \sample [0, 2^\lambda)$. If a trusted setup is acceptable then $\gr{g}, \gr{h}$ can be chosen such that $\langle \gr{g} \rangle = \langle \gr{h} \rangle$ generate the same subgroup (e.g. set $\gr{h} = \gr{g}^\alpha$ for large random $\alpha$). In this case the commitment is statistically hiding~\cite{AC:DamFuj02}. 

The setup can be made publicly verifiable using a hash function onto the group $\GG$, \emph{i.e.}, a collision-resistant function $H: \{0,1\}^* \rightarrow \GG$ that behaves as a random oracle. In this case the commitment scheme is computationally hiding under a \emph{subgroup indistinguishability assumption}, as formulated by Brakerski and Goldwasser~\cite{C:BraGol10}. A subgroup $\GG' \subseteq \GG$ is computationally indistinguishable from $\GG$ if $\gr{g}' \sample \GG'$ is computationally indistinguishable from $\gr{g} \sample \GG$. A sufficient assumption that would imply Pedersen commitments with the random oracle setup are computationally hiding is that for $\gr{h} \sample \GG$, the subgroup $\langle \gr{h} \rangle$ is indistinguishable from $\GG$. This basic subgroup indistinguishability assumption holds in generic groups of unknown order. 
