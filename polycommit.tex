...
The interactive argument is called \textbf{public-coin} if the all the verifier's messages are uniformly random values, independent of all prior messages and the setup parameters $\params$. 
...
Given a public-coin interactive argument tuple $(\textsf{Setup}, P, V)$ and arbitrary prover algorithm $P^*$, let $\textsf{Record}(P^*, \params, x, $\sout{$w$}$\textcolor{blue}{\st})$ denote the message transcript between $P^*$ and $V$ on shared input $x$, \sout{witness $w$} \textcolor{blue}{initial prover state $\st$}, and $\params$ generated by $\textsf{Setup}$. Furthermore, let $E^{\textsf{Record}(P^*, \params, x, \textnormal{\sout{w}}\textcolor{blue}{\st})}$ denote an machine $E$ with a transcript oracle for this interaction that can be rewound to any round and run again on fresh verifier randomness. The tuple $(\textsf{Setup}, P, V)$ has witness-extended emulation if for every deterministic polynomial time $P^*$ there exists an expected polynomial time emulator $E$ such that for all non-uniform polynomial time adversaries $\mathcal{A}$ the following condition holds: 
\[
\Pr \left[
...
\begin{array}{c}
             \params \leftarrow \textsf{Setup}(1^\lambda) \\
             (x, \textnormal{\sout{$w$}}\textcolor{blue}{\st}) \leftarrow \mathcal{A}(\params) \\
             \textsf{tr} \leftarrow \textsf{Record}(P^*, \params, x, \textnormal{\sout{$w$}}\textcolor{blue}{\st})
\end{array} 
\right] \approx
\]
\[
...
\begin{array}{c}
             \params \leftarrow \textsf{Setup}(1^\lambda) \\
             \textcolor{blue}{(}x\textcolor{blue}{, \st)} \leftarrow \mathcal{A}(\params) \\
(\textsf{tr}, w^\textnormal{\sout{$*$}}) \leftarrow E^{\textsf{Record}(P^*, \params, x, \textnormal{\sout{$w$}}\textcolor{blue}{\st})}(\params, x)
\end{array}
\right]
\]
...
A commitment scheme $\Gamma$ is a tuple $\Gamma = (\pro{Setup}, \pro{Commit}, \pro{Open})$ of protocols where
\begin{itemize}
    \item $\pro{Setup}(1^\lambda) \rightarrow \params$ generates public parameters $\params$;
    \item $\pro{Commit}(\params; x, r) \rightarrow (c; d)$ takes a secret message $x$ (and, optionally, secret randomness $r$) and outputs a public commitment $c$ and secret opening hint $d$.
    \item $\pro{Open}(\params, c, x; d) \rightarrow b \in \{0, 1\}$ verifies the opening of commitment $c$ to the message $x$ provided with the opening hint $d$.
\end{itemize}
...
\begin{itemize}
    \item $\pro{Eval}(\params, c, z, y, d; f(x)) \rightarrow b \in \{0, 1\}$ is a protocol that takes a public commitment $c$, point $z \in R$, and a value $y$, degree $d$, along with a secret polynomial $f(x) \in R[x]$ with $\deg(f(x)) \leq d$ to which $c$ is a commitment, and shows that the evaluation of the secret committed polynomial $f(x)$ at $z$ is indeed $y$.
\end{itemize}
...
\begin{itemize}
	...
	\item \begin{enumerate}
		\item  $y_i\gets \frac{y \mod q^{i+1}-y \mod q^{i}}{q^i} \forall i \in [0,\lfloor\log_q(y)\rfloor]$
		\item $f_i=y_i$ if $y_i<\frac{q}{2}$ else $f_i=y_i-q$
		\item \benedikt{Ben fill in the decoding algorithm}
	\end{enumerate} 
\end{itemize}
...
\begin{minipage}{\textwidth}
	...
	$\pro{Commit}(\crs;f(x)):$ \pccomment{$f(x) \in \mathbb{Z}[x]$ with coefficients in $[0;p-1]$}
		...
		\end{enumerate}
	$\pro{Open}(\crs,\gr{C},\bar{f}(x);f(x)):$ \pccomment{$f(x) \in B_{q/2}[X]\subset\mathbb{Z}[x]$ but $\bar{f}(x) \in \mathbb{F}_p[x]$}
		\begin{enumerate}[nolistsep]
