...
In each round the extracted witness is a dyadic rational $\hat{y}$ such that $g^{\hat{y}}=C$ and $\hat{y}=\enc(f(X))$ where the coefficients of $f(X)$ are in $C_{b_n,b_d}$, i.e. have bounded numerators and denominators. The degree of $f(X)$ is at most $\hat{d}$. Also $f(z) \equiv y \bmod p$.
If $\hat{d}=0$ then we can directly extract the constant integer polynomial $f(X)=\hat{y} \in \ZZ$ such that $\vert \hat{y} \vert < p^{\log_2(d+1)+1}$, $y=\hat{y}\mod p$, $f(X)=y\in \ZZ_p[X]$ and $g^{\hat{y}}=C$ as the witness. We proceed with $d'=2\cdot d +1$.
