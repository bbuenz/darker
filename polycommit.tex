...
In our running example, $f_L(X)=4x+1$ and $f_R(X)=2x+3$. Note that the verifier can check the consistency of these commitments by testing $\gr{G}_L\gr{C}_R^{q^{d'+1}}=\gr{C}$. Now we reduce the problem to a smaller statement by taking a random linear combination between $\gr{C}_L$ and $\gr{C}_R$, \emph{i.e.}, $\gr{C}'\gets \gr{C}_L^{\alpha}\gr{C}_R=\gr{g}^{(\alpha f_L+f_R)(q)}$. Now we can recurse on $\gr{C}'$ with the statement that it commits to a degree $d'$ polynomial. 
After $\log_2(d+1)$ steps $\gr{C}'$ is a commitment to a constant $f$ which the prover can send to the verifier. 
The verifier checks that $\gr{g}^f=\gr{C}'$ and also that $f \ll q$.
This ensures that a) $f$ is decodable to a degree $0$ polynomial and additionally, through an extraction argument, that the original $f(X)$ was a degree $d$ polynomial with bounded integer coefficients.
