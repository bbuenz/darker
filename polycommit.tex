...
The purpose of the commitment function is to bind the prover to one particular polynomial for every commitment held by the verifier. In particular, at the start of every recursion step, the verifier is in possession of a commitment $[\![f(q)]\!]$ to $f(q)$. The prover provides commitments $[\![f_L(q)]\!]$ and $[\![f_R(q)]\!]$, and the verifier checks their soundness homomorphically via $[\![f(q)]\!] = [\![f_L(q)]\!] + q^{d'+1} \times [\![f_R(q)]\!]$.
