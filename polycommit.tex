...
\paragraph{Decoding}
Decoding an integer $y$ works as follows. Define the partial sum $S_k := \sum_{i=0}^k f_i q^i$ with $S_{-1} := 0$ and observe that when $(S_k \bmod q^{k+1}) > q^{k+1}/2$ then $S_k < 0$ whereas when $(S_k \bmod q^{k+1}) < q^{k+1}/2$ then $S_k \geq 0$. 
...
% \left\{ \begin{array}{ll}
%\hat{S}_k & \textnormal{if } \ \hat{S}_k < q^{k+1}/2 \\
%q^{k+1} -\hat{S}_k & \textnormal{if } \ \hat{S}_k > q^{k+1}/2 \\
%\end{array} \right.$. 
...
\benedikt{Integer root fractional root}
