...
\begin{minipage}{\textwidth}
	...
	$\pro{PoKE2}(\gr{u}, \gr{w}; x):$
		\begin{enumerate}[nolistsep]
		    \item \verifier samples $\gr{g} \sample \mathbb{G}$ and sends it to \prover
		    \item \prover computes $\gr{Z} \gets \gr{g}^x$ and sends it to \verifier
		    \item \verifier samples $\ell \sample \primes$ and $\alpha \sample [0;2^\lambda-1)$ and sends $(\ell, \alpha)$ to \prover
		    \item \prover computes quotient $q$ and remainder $r$ such that $x = q\ell + r$ and $r \in \{0, \ldots, \ell-1\}$
...
\begin{itemize}
	...
	\begin{enumerate}[nolistsep]
		...
		\item  Compute $f_{\leq i}(q)\gets \begin{cases}y \bmod q^{i+1} & \text{ if }y \bmod q^{i+1}<q^{i+1}/2\\
...
\begin{lemma}
	The polynomial commitment scheme is correct for polynomials in $\ZZ_p$ of degree at most $d$ if $q> p^{3\lceil \log_2(d+1)\rceil+1}$.
\end{lemma}
\begin{proof}
	We need to ensure that $q$ is larger than $f$ the final value sent by the prover. We show this by showing that $b$ is a bound  In each round the bound on the coefficient size grows by a factor of $p$, for example: If $f'_0=\alpha f_0 + f_1$ for $\alpha \in [0,p-1]$ and $f_0,f_1 \in [0,b]$ then $f'_0\in [0,p\cdot b]$.
...
\end{lemma}
\begin{proof}
    Assume that there is an adversary that breaks the binding property of the scheme. Specifically, assume that some probabilistic polynomial time algorithm $\adv$ takes as input $\params$ and outputs $\gr{C} \in \GG, f(X) \in B_{q}[X], f'(X)\in B_{q}[X]$ such that with non-negligble probability $\pro{Open}(\params, \gr{C}, f(X)) = \pro{Open}(\params, \gr{C}, f'(X)) = 1$ and $f(X) \neq f'(X)$.\alan{$\mathsf{Open}$ takes 4 arguments not 3. It needs both an element from $\mathbb{Z}_p[X]$ and one from $\mathbb{Z}[X]$.} We proceed to show that this implies a violation of the order assumption~(Assumption \ref{assum:order}) and the strong RSA assumption~(Assumption \ref{assum:strongRSA}). The assumptions are incomparable so we show that either suffices to achieve the binding property of the commitment scheme.
