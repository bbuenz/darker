...
$\ZZ_n$ is as hard as factoring $n$. Unfortunately, the order assumption does not hold for $\ZZ_n$ this is because $-1 \in \ZZ_n$ can be easily computed and has order two. This can be resolved though by working in the quotient group $\ZZ_n\{-1,1\}$ which does not contain an element of order $2$. Alternatively one can use the subgroup of quadratic residues $\mathbb{QR}\subset \ZZ_n$ which also does not contain elements of known order. Additionally if $n$ is the product of strong primes, i.e. $\frac{p-1}{2}$ and $\frac{q-1}{2}$ are prime, then $\mathbb{QR}\subset\ZZ_n$ does not contain elements of low order\cite{JC:FisSch00,C:HofKil09}. In this group the order assumption and the strong RSA assumption are equivalent to the hardness of factoring $n$.

Generating $n$ requires a private coin trusted setup as the factorization of $n$ needs to be unknown. To circumvent this we can use the class groups of imaginary quadratic order \cite{Fill in} which are candidate groups of unknown order but do not require a trusted setup. Specifically, we can define a class group $\mathbb{CL}(p)$ by sampling a prime discriminant $p$. $p$ can be public which means that the setup simply requires random bits for sampling $p$. If $p=7 \bmod 8$ then we know that the order of $p$ is odd and that no elements of order $2$ exist.
Working in $\mathbb{CL}(p)$ presents us with several difficulties. Most importantly, there exists an efficient polynomial time algorithm due to Gauß and described by Bosma and Stevenhagen\cite{bosma1996computation} to compute square roots of arbitrary elements in $\mathbb{CL}(p)$. This algorithm can be leveraged to compute arbitrary power of two roots. 
Class groups can therefore not be used to cryptographically commit to integers but rather to rationals, whose denominator is a power of $2$. This set of rationals is called the \emph{dyadic rationals}. 
...
\end{lemma}
The proof of the Lemma is nearly identical to the proof of Lemma \ref{lem:strongtofractional}
