...
: 
\begin{array}{l} 
f \in R[X] \ \text{has degree at most} \ d \ \text{and} \ f(z) = y \\ 
...
\paragraph{Commitment to dyadic rationals}
In class groups there exists an algorithm to compute square roots of any element originially described by Gaus (modern description can by Bosma and Stevenhagen.\cite{bosma1996computation}). This means that in class groups an adversary can also commit to numbers in the ring of dyadic rationals $\mathcal{D}:=\{\frac{x}{2^k} |x \in \ZZ \wedge k \in \NN\}$. We call elements in $\mathcal{R}_{2^k}$ 2-fractions. When using class groups we need to extend the encoding scheme from supporting integers to dyadic rationals. The main difference to the integer encoding scheme will be that the scheme works for dyadic rationals such that for $\frac{x}{2^a}\in \mathcal {D}$ $|x|<\sqrt{q}/2$ and $2^a<\sqrt{q}$. For simplicity assume that $q=2^{2b+2}$ for some integer $b$. Let $C_{q=2^{2b+2}}:=\{(x,a)\in \mathcal{R}_{2^a} | |x|<2^b \wedge 2^a\leq 2^{b+1}\}$ denote the set of such ``small'' dyadic rationals. To encode a polynomial $g(X)$ with coefficients in $C_{q}$ we compute $g(q)$. For decoding we simply compute the integer $y'\cdot 2^{b+1}\in \ZZ$  and use the decoding algorithm described above to decode a polynomial $f(X)$ in $B_{q}[X]$. From $f(X)$ we can derive the polynomial  $g(X)\gets \frac{f(X)}{2^{b+1}} \in C_q[X]$ through division.
...
\begin{minipage}{\textwidth}
	    ...
	    \begin{enumerate}[nolistsep]
        \item \pcif $d=0$:
        \item \pcind[1] \prover sends $f_0=f(X)$ to the verifier. \pccomment{$f(X)$ is a constant}
        ...
        \item \pcind[1] \verifier checks that $f_0 \in [0,b]$
          \item \pcind[1] \verifier checks that $f_0\equiv y \bmod p$
                \item \pcind[1] \verifier checks that $\gr{g}^{f_0}=\gr{C}$
...
\end{lemma}
\begin{proof}
	We need to ensure that $q$ remains larger than the bound $b$ and the opened polynomial. In each round the coefficients grow by a factor of at most $p$, e.g. $f'_0=f_0+\alpha f_1$ for $\alpha \in [0,p-1]$ and $f_0,f_1 \in [0,b-1]$. The recursion has $\lceil \log_2(d+1)\rceil$ rounds which gives us a bound $b$ of $p^{\lceil \log_2(d+1)\rceil+1}$. If $f(z)=y \bmod p$ then $f'(z)=y' \bmod p$. $\textsf{PoE}$ itself is a perfectly complete argument. The rest of the protocol's correctness is immediate.
\end{proof} 
...
In each round the extractor has $f'(X)=f_L(X)+\alpha f_R(X)$. Using rewinding the extractor can also compute $f''(X)=f_L(X)+\alpha' f_R(X)$. From $f'(X)$, $f''(X)$, $\alpha$ and $\alpha'$ it is easy to compute $f_L(X)$ and $f_R(X)$. A careful analysis shows that if the coefficients of $f'(X)$ are bounded by $b$ then $f_L(X)$ and $f_R(X)$ must have coefficients bounded by $b \cdot p$ in absolute value. Using a similar analysis we can show that $f(z)\bmod p=y$ for the extracted polynomial $f(X)$. Our full proof takes into account the cryptographic compilation of the protocol using the integer encoding and exponentiation in groups of unknown order. Additionally the full proof will need to work with dyadic rationals as taking square roots is easy in class groups. 
\begin{theorem}
	The polynomial commitment scheme for polynomials in $\ZZ_p[X]$ instantiated using $\ggen$ is extractable if the adaptive root assumption and the strong RSA assumption hold for $\ggen$ if $q>2 \cdot p^{2 \lceil \log_2(d) \rceil+1}$
\end{theorem}
\begin{proof}
We will proof security by showing that given a polynomial time adversary $\adv_{\eval}$ that succeeds in convincing an honest verifier in the $\eval$ protocol on any public input with non-negligible probability we can either (1) construct an adaptive root adversary $\adv_{\textsf{Adaptive Root}}$, (2) extract an element of known order, i.e. break the order assumption, (3) extract a fractional root of $\gr{g}\in \GG$ or (4) extract the polynomial $f(X)$. The proof will use the general forking lemma (Lemma \ref{lem:forking}) to show that the polynomial commitment scheme has witness-extended emulation.

In particular we construct an extractor $E$ that given transcripts with $2$ distinct challenges per round, i.e. $2^{\lceil\log_2(d+1)\rceil}<2 (d+1)$ total transcripts, can compute either an opening to the commitment scheme, an element of known order or a pseudo root of the CRS encoded $\gr{g}\in\GG$.
...
We will now construct the extractor that recursively either extracts the encoding of a dyadic rational polynomial $f(X)\in \mathcal{D}[X]$ with bounded coefficients or a break of the order assumption or the pseudo-root assumption. The recursion is done over the degree $\hat{d}$ up to the maximum degree $d$.

In each round the extracted witness is a dyadic rational $\hat{y}$ such that $g^{\hat{y}}=C$ and $\hat{y}=\enc(f(X))$ where the coefficients of $f(X)$ have bounded numerators and denominators. The degree of $f(X)$ is at most $\hat{d}$. Also $f(z) \equiv y \bmod p$.
If $\hat{d}=0$ then we can directly extract $f(X)=\hat{y} \in \ZZ$ such that $\vert \hat{y} \vert < p^{\log_2(d+1)+1}$, $y=\hat{y}\mod p$, $f(X)=y\in \ZZ_p[X]$ and $g^{\hat{y}}=C$ as the witness. We proceed with $d'=2\cdot d +1$.

For $d'>0$ we have $g^{\hat{y}}=C=C_0^{\alpha}C_1$ and $g^{\hat{y}'}=C=C_0^{\alpha'}C_1$ for distinct $\alpha$ and $\alpha'$.  
 This gives us $C_0^{\alpha-\alpha'}=g^{\hat{y}-\hat{y}'}$. 
 If $\frac{\alpha-\alpha'}{\gcd(\alpha-\alpha',\hat{y}-\hat{y}')}$ is not a power of $2$ then this gives us a pseudo-root of $g$. Under the pseudo-root assumption which follows from the strong-RSA and the order assumption this happens with probability $\advantage{\textsf{Pseudo-root}}{}$ which is negligible. If this is not the case then we can compute $D=g^{\frac{\hat{y}-\hat{y}'}{\alpha-\alpha'}}$. Either $D=C_0$ or $(D/C_0)^{\alpha-\alpha'}=1$, i.e. $D/C_0$ is an element of known order.
...
Finally, if no bad event occurs and no assumption is broken we have $C_0=g^{\hat{y}_0}$ where $\hat{y}_0=\frac{\hat{y}-\hat{y}'}{\alpha-\alpha'}$ is a dyadic rational.  
 
 In that case $C_1=g^{\hat{y}_1}=g^{\hat{y}-\alpha\hat{y}_0}$. The extractor has now successfully obtained $\hat{y}_0\in \mathbb{D}$ and $\hat{y}_1\in \mathbb{D}$.
...
\begin{minipage}{\textwidth}
	\begin{flushleft}
	$\pro{ExtractCoefficient}(\params, \gr{C}, i, \gr{C}_a; f(X)):$ \pccomment{$f(X) \in \ZZ[X]$}
		\begin{enumerate}[nolistsep]
...
\begin{lemma}[security of Protocol~\ref{scheme:coefficient_extraction}]
    relation and witness-extended emulation of Protocol~\ref{scheme:coefficient_extraction}. Much wow.
\end{lemma}

\begin{figure}[!htp]
\renewcommand{\figurename}{Protocol}
\caption{OpenIndex} \label{scheme:homomorphic_openindex}
\noindent\begin{mdframed}[userdefinedwidth=\textwidth]
\begin{minipage}{\textwidth}
	\begin{flushleft}
	$\pro{OpenIndex}(\params, \gr{C}, a, i; f(X)):$ \pccomment{$f(X) \in \ZZ_p[X]$}
		\begin{enumerate}[nolistsep]
		    \item \prover computes $\gr{C}_a \gets \gr{c}^{f_i}$ and sends it to \verifier
		    \item \prover and \verifier run $\pro{ExtractCoefficient}(\params, \gr{C}, i, \gr{C}_a; f(X))$
...
\begin{lemma}[security of Protocol~\ref{scheme:homomorphic_openindex}]
    wee
\end{lemma}
...
\begin{minipage}{\textwidth}
	\begin{flushleft}
	$\pro{InnerProductSecret}(\params, \gr{C}_\mathbf{f}, \gr{C}_a; \mathbf{f}, \mathbf{s}):$ \pccomment{$\mathbf{f}, \mathbf{s} \in \ZZ^{d+1}$}
		\begin{enumerate}[nolistsep]
		    ...
		    \item \prover and \verifier run $\pro{ExtractCoefficient}(\params, \gr{C}', d, \gr{C}_a; h(X))$
		    \item \pcif{}all checks pass \textbf{then} \textbf{return} 1 \textbf{else} \textbf{return} 0
		\end{enumerate}
		\vspace{0.25cm}
	$\pro{InnerProductCommitted}(\params, \gr{C}_\mathbf{f}, \gr{C}_a, \gr{C}_\mathbf{\bar{s}}; \mathbf{f}, \mathbf{s}):$ \pccomment{$\mathbf{f}, \mathbf{s} \in \ZZ^{d+1}$ and $\gr{C}_\mathbf{\bar{s}} = \gr{g}^{\sum_{i=0}^d s_{d-i} q^i}$}
		\begin{enumerate}[nolistsep]
		    \item \prover computes $\gr{C}' \gets \gr{C}_\mathbf{f}^{\sum_{i=0}^{d} s_{d-i} q^i}$ and sends it to \verifier
		    \item \prover computes $h(X) \gets \left(\sum_{i=0}^d f_i X^i\right) \left(\sum_{i=0}^d g_{d-i} X^i\right)$
		    \item \prover and \verifier run $\pro{PoKEE}(\gr{g}, \gr{C}_\mathbf{\bar{s}}, \gr{C}_\mathbf{f}, \gr{C}'; \mathbf{s})$
		    \item \prover and \verifier run $\pro{ExtractCoefficient}(\params, \gr{C}', d, \gr{C}_a; h(X))$
		    \item \pcif{}all checks pass \textbf{then} \textbf{return} 1 \textbf{else} \textbf{return} 0
		\end{enumerate}
		\vspace{0.25cm}
	$\pro{InnerProductKnown}(\params, \gr{C}_\mathbf{f}, \gr{C}_a, \mathbf{s}; \mathbf{f}):$ \pccomment{$\mathbf{f}, \mathbf{s} \in \ZZ^{d+1}$}
		\begin{enumerate}[nolistsep]
		    \item \prover computes $\gr{C}' \gets \gr{C}_\mathbf{f}^{\sum_{i=0}^{d} s_{d-i} q^i}$ and sends it to \verifier
		    \item \prover computes $h(X) \gets \left(\sum_{i=0}^d f_i X^i\right) \left(\sum_{i=0}^d g_{d-i} X^i\right)$
		    \item \prover and \verifier run $\pro{ExtractCoefficient}(\params, \gr{C}', d, \gr{C}_a; h(X))$
