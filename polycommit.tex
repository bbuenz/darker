...
\]
\end{assumption} 
We note that some definitions of the Strong RSA Assumption additionally require that $\ell$ be an odd prime~\cite{EC:BarPfi97}. Our definition is stronger a stronger assumption which states that all roots are difficult.
...
In addition to the previous assumptions, we define the Fractional Root Assumption, which states that for random group elements $\gr{g}$ it is hard to find a tuple $(\gr{u}\in \GG,\alpha\in \ZZ,\beta\in \ZZ)$ such that $\gr{u}^{\beta}=\gr{g}^{\alpha}$. 
...
\begin{assumption}[Non-Dyadic Root Assumption]
\label{assum:dyadicfracroot}
The \defn{Non-Dyadic Root Assumption} holds for $\ggen$ if for any efficient adversary $\adv$:
\[        
                \Pr\left[\gr{u}^\beta = \gr{g}^{\alpha} \wedge \frac{\beta}{\gcd(\alpha,\beta)}\neq 2^k,  k \in \NN   : 
                \begin{array}{l} 
                      \GG \sample \ggen(\lambda) \\ 
...
\begin{lemma}
\label{lem:dyadicstrongtorational}
	The Adaptive Root Assumption and the Dyadic Strong RSA Assumption imply the Non-Dyadic Root Assumption.
\end{lemma}
...
\paragraph{Encoding}
For a ``large'' integer $q$, the map $\mathsf{Enc} : \mathbb{Z}((q-1)/2)[X] \rightarrow \mathbb{Z}, \, p(X) \mapsto p(q)$ is an injective map from the domain of polynomials with bounded coefficients to the integers. This means the map can be used as encoding of polynomials in the domain $\ZZ(b)[X]$ with $b<q/2$.
\paragraph{Decoding}
...
Before we present our concrete polynomial commitment scheme based on groups of unknown order, we present an abstraction thereof that remains faithful to the mechanics of the integer encoding, but hides the concrete cryptographic steps. The purpose of this abstraction is two-fold: first, it provides an intuitive stepping stone from which presenting and studying the concrete cryptographic protocol is easier; and second, it opens the door to alternative cryptographic instantiations that provide the same interface but based on alternative hardness assumptions.

Let $[\![ \cdot ]\!] : \mathbb{Z} \rightarrow \mathbb{S}$ be a linearly homomorphic hash function that sends integers to elements of some set $\mathbb{S}$. Moreover, let $\mathbb{S}$ be equipped with operations $+$ and $\times$ that accomodate the homomorphism for $[\![ \cdot ]\!]$. Specifically, for all $a, b, z_0, z_1 \in \mathbb{Z}$, we have $a \times [\![z_0]\!] + b \times [\![z_1]\!] = [\![az_0 + bz_1]\!].$ (Later on, we will identify $\mathbb{S}$ with a group $\mathbb{G}$ of unknown order.)

With this homomorphic hash function and with the integer encoding scheme of the previous subsection, we construct a commitment function to polynomials that sends $\mathbb{Z}_p \rightarrow \mathbb{S}, f(X) \mapsto [\![f(q)]\!]$. This commitment function inherits the homomorphic properties of the integer encoding for a limited number of additions and multiplications-by-constant. With this commitment function we are in a position to present the abstract evaluation protocol.
...
The prover splits $f(X)$ into $f_L(X)$ and $f_R(X)$ such that $f(X) = f_L(X) + X^{d'+1} f_R(X)$ and such that both halves have degree at most $d'$. He obtains a random challenge $\alpha\in [-\frac{p-1}{2},\frac{p-1}{2}]$ from the verifier and proceeds to prove that $f'(X)=\alpha \cdot f_L(X) + f_R(X)$ has degree $d'$ and that $f'(z) = y' = \alpha y_L + y_R$ with $y_L = f_L(z)$ and $y_R = f_R(z)$. 

The proof recursively repeats this reduction by using $f'(X),z,y'$ and $d'$ as the input. In the final step, $f(X) = f$ is a constant and the verifier checks that $f \equiv y \bmod p$. Note that $|f| <  (\frac{p}{2})^{\log_2(d+1)+1}$ so a representation of $f$ requires at most $\lceil \log_2(d+1) \cdot \log_2(p)\rceil$ bits.

The purpose of the commitment function is to bind the prover to one particular polynomial for every commitment. To this end, the verifier must be in possession of a commitment 
...
\begin{minipage}{\textwidth}
	...
	$\pro{Setup}(1^\secpar):$
		...
		\end{enumerate}
	$\pro{Commit}(\crs;f(X) \in \ZZ(p)[X]):$ \pccomment{$f(X)\equiv \bar{f}(X) \mod p$ for  $\bar{f}(X)\in \ZZ_p[X]$}
		\begin{enumerate}[nolistsep]
			\item $\gr{C} \gets \gr{g}^{f(q)}$
			\item $\pcreturn (\gr{C};f(X))$
		\end{enumerate}
	$\pro{Open}(\crs,\gr{C}f(X)):$ \pccomment{$f(X) \in \ZZ(b)[X]\subset\mathbb{Z}[X]$ for $b<q/2$}
		\begin{enumerate}[nolistsep]
...
\paragraph{Hiding and Zero-knowledge.} Turning the polynomial commitment into a hiding scheme is easy by replacing $g^{\bar{f}(q)}$ with a Pedersen commitment to $\bar{f}(q)$ over $\GG$. The setup parameters must contain two independent generators $\gr{g}, \gr{h}$ for which the discrete logarithm between $\gr{g}$ and $\gr{h}$ is unknown. (Previously, in the non-hiding scheme, any generator would suffice). A commitment to $f \in \FF_p$ is then $\gr{g}^{\bar{f}(q)} h^r$ for random $r \sample [0, 2^\lambda)$. If a trusted (or MPC) setup is acceptable then $\gr{g}, \gr{h}$ can be chosen such that $\langle \gr{g} \rangle = \langle \gr{h} \rangle$ generate the same subgroup (e.g. set $\gr{h} = \gr{g}^\alpha$ for large random $\alpha$). In this case the commitment is statistically hiding~\cite{AC:DamFuj02}. 

The setup can be made publicly verifiable using a hash function onto the group $\GG$, i.e. a collision-resistant function $H: \{0,1\}^* \rightarrow \GG$ that behaves as a random oracle. In this case the commitment is computationally hiding under a \emph{subgroup indistinguishability assumption}, as formulated by Brakerski and Goldwasser~\cite{C:BraGol10}. A subgroup $\GG' \subseteq \GG$ is computationally indistinguishable from $\GG$ if $\gr{g}' \sample \GG'$ is computationally indistinguishable from $\gr{g} \sample \GG$. A sufficient assumption that would imply Pedersen commitments with the random oracle setup are computationally hiding is that for $\gr{h} \sample \GG$, the subgroup $\langle \gr{h} \rangle$ is indistinguishable from $\GG$. This basic subgroup indistinguishability assumption holds in generic groups of unknown order. 

In class groups we need to be careful applying this subgroup indistinguishability assumption because Gauss's square root algorithm may be used to test quadratic residuosity efficiently. As a result, it is possible to distinguish the subgroup of quadratic residues from non-quadratic residues. However, the class group can be constructed such that the order is guaranteed to be odd~\cite{PKC/BucHam01}, in which case every element has a $2^n$ root for any $n \geq 0$.% For class groups, we assume the subgroup indistinguishability assumption applies in $NQR$, the subgroup of non-quadratic residues. It is easy to convert the deterministic $H$ into a PPT hash function onto $NQR$ by iterating until hitting the first quadratic non-residue (succeeding in an expected constant number of iterations). 
...
\end{lemma}
\begin{proof}
\benedikt{Very similar to Lemma \ref{lem:intrandomcombine}}
...
$\mathcal{R}_{\pro{JoinedEval}} = \left\{ \langle(\crs,\gr{C}_1, \gr{C}_2 \in \GG, z,y_1,y_2 \in \ZZ_p,b,d \in \NN), f_1(X),f_2(X) \in \ZZ(b)[X] \rangle \ : \ f_1(z)=y_1 \bmod p\wedge f_2(z)=y_2 \bmod p\wedge  \gr{C}_1=\gr{g}^{f_1(q)}\wedge \gr{C}_2=f_2(q) \wedge \deg(f_1)\leq d \wedge \deg(f_2)\leq d \right\}$.
...
Statment: $(\crs,\gr{C}_1,\gr{C}_2,z,y_1,y_2,b,d)\in \mathcal{R}_{\pro{JoinedEval}}$
        ...
        \item \verifier samples $\alpha \sample [-\frac{p-1}{2},\frac{p-1}{2}]$ and sends it to \prover
			...
			\item \prover and \verifier run $\pro{EvalBounded}(\params,\gr{C}',z,y',d,\frac{p-1}{2}^2;f(X))$
		    \end{enumerate}
\end{mdframed}

\begin{theorem}
The protocol $\pro{JoinedEval}$ is an interactive argument for the relation $\mathcal{R}_{\pro{JoinedEval}}$ and has perfect completeness and witness extended emulation if $q>$
\end{theorem}
...
As discussed it has highly batchable proofs and it is possible to evaluate $n$ degree $d$ polynomials at $m$ points using only $2\log_2(d+1)$ group elements and $(m+1)\log_2(d+1)$ field elements. Note that this means the proof size is independent of $n$ and linear in $m$ but with a small constant $(15 \log(d)$ bytes).
...
\begin{minipage}{\textwidth}
	...
	$\pro{MultiSetup}(1^\secpar):$
		\begin{enumerate}[nolistsep]
			...
			\item $\pcreturn \params = (\secpar,\GG,\gr{g},p,q)$
		\end{enumerate}
	$\pro{MultiCommit}(\crs;f(X_1,\dots,X_n) \in \ZZ(\frac{p-1}{2})^n[X]\subset \ZZ[X]):$ 		\begin{enumerate}[nolistsep]
...
\end{mdframed}
 ...
 \begin{mdframed}
\begin{minipage}{\textwidth}
			$\pro{MultiEval}(\crs, \gr{C}\in \GG, \boldsymbol{z}\in \ZZ_n,y \in \ZZ_p, d,n,b \in \NN; f(X_1,\dots,X_n)\in \ZZ(b)^n[X]) :$ 			\begin{enumerate}[nolistsep]
			\item \pcif{n=1} 
