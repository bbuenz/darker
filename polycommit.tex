...
Using the integer encoding of polynomials presented above we can simply commit to an integer polynomial with bounded coefficients $f(X)\in B_{q/2}[X]$ by computing $\gr{g}^{f(q)} \in \GG$. 
...
The commitment inherits the homomorphic properties of the integer encoding. Assume that we are committing to polynomials in $\FF_p[X]$, i.e. polynomials with coefficients bounded by $p$. Then commitment scheme supports up to $\frac{q}{2 \cdot p}$ homomorphic additions. We use this to build an efficient $\eval$ protocol. 
...
\begin{mdframed}
\begin{minipage}{\textwidth}
			$\pro{Eval}(\crs, \gr{C}\in \GG, z\in \FF_p, y\in \FF_p, d \in \NN; f(X)) :$ \pccomment{$f(X) = \sum_{i=0}^d f_i X^i \in \FF_p[X]$}\\
			 \prover and \verifier run $\pro{EvalBounded}(\crs,z,y,d,p-1;f(X))$\\
		$\pro{EvalBounded}(\crs,\gr{C}\in \GG,z\in \FF_p,y\in \FF_p,d\in \NN,b\in \ZZ;f(X)\in B_{b}[X])$
	    \begin{enumerate}[nolistsep]
        \item \pcif $d=0$:
...
\begin{minipage}{\textwidth}
	\begin{flushleft}
	$\pro{InnerProduct}(\params, \gr{C}_\mathbf{f}, \gr{C}_a; \mathbf{f}, \mathbf{s}, \hat{\mathbf{f}}):$ \pccomment{$\mathbf{f}, \mathbf{s} \in \mathbb{F}_p^{d+1}$ but $\hat{f}(X) \in \mathbb{Z}[X]$}
		\begin{enumerate}[nolistsep]
