...
In each round the extracted witness is a dyadic rational $f_i(q)\in \mathbb{D}$ such that $\gr{g}^{f_i(q)}=\gr{C}_i$ and $f_i(X)=\dec(f_i(q))$ such that the coefficients of $f_i(X)$ are in $C_{b_n,b_d}$, i.e. have bounded numerators and denominators. The degree of $f_(X)$ is at most $d_i$ and $f(z) \equiv y \bmod p$. Note that for odd primes $p$ and integer $z$ $f(z)\bmod p$ is always defined.
...
We now show how to compute the witness for $i-1$ given a witnesses for $i$. If $d_i+1$ is odd then we have $\gr{C}_{i-1}=\gr{C}_i^q$. Since $\gr{C}_{i-1}=\gr{g}^{f_{i-1}(q)}$ we either have that $q$ divides $f_{i-1}(q)$ or since $q$ is odd we have a pseudo root of $\gr{g}$. 
