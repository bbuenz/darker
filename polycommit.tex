...
The commitment function binds the prover to one particular polynomial for every commitment held by the verifier. In particular, at the start of every recursion step, the verifier is in possession of a commitment $[\![f(q)]\!]$ to $f(X)$. The prover provides commitments $[\![f_L(q)]\!]$ and $[\![f_R(q)]\!]$, and the verifier checks their soundness homomorphically via $[\![f(q)]\!] = [\![f_L(q)]\!] + q^{d'+1} \! \times \! [\![f_R(q)]\!]$. From these commitments, the verifier can also compute the commitment to $f'(X)$ homomorphically, via $[\![f'(q)]\!] = \alpha \! \times \! [\![f_L(q)]\!] + [\![f_R(q)]\!]$. In the last step, the verifier checks that the constant polynomial $f$ matches the commitment by computing $[\![f]\!]$ outright.
