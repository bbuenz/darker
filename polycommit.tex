...
$q$ needs to be large enough such that $f_0(X)$ is uniquely decodable, i.e. $q>2\cdot b=(p-1)^{\lceil \log_2(d+1)\rceil+1}(\frac{p+1}{2})^{\lceil \log_2(d+1)\rceil}$.
...
\begin{minipage}{\textwidth}
			...
			\item \pcif{n=1} 
			\item \pcind[1] \prover and \verifier run $\pro{EvalBounded}(\params,\gr{C},z_1,y,d,\frac{p-1}{2}^nx;f(X_1))$ 
			\item \pcelse
			...
			\item \pcind[1]\prover and \verifier run $\pro{MultiEval}(\crs,C',z_1,\dots,z_{n-1},y',d,n-1;f')$
		    \end{enumerate}
      \end{minipage}
\end{mdframed}
