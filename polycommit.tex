...
\begin{lemma}
	The polynomial commitment scheme is correct for polynomials in $\ZZ_p$ of degree at most $d$ if $q>  (\frac{p-1}{2})^{\lceil \log_2(d+1)\rceil+1}$.
\end{lemma}
\begin{proof}
...
Now if $f$ and $f'\in[-b,b]$ and $\alpha,\alpha' \in [-\frac{p-1}{2},\frac{p-1}{2}]/\{0\}$ then by triangle inequality $f_L\in [-2b,2b]$. Additionally we have $f_R=\frac{f'\alpha-f \alpha'}{\Delta_\alpha}$. Using the triangle inequality again we have that $f_R \in [-(p-1) \cdot b, (p-1) \cdot b]$.
%The actual bound on f_R is b*(p-2)
