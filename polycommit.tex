...
\begin{minipage}{\textwidth}
			$\pro{Eval}(\crs, \gr{C}\in \GG, z\in \FF_p, y\in \FF_p, d \in \NN; f(X)) :$ \pccomment{$f(X) = \sum_{i=0}^d f_i X^i \in \FF_p[X]$}\\
			 \prover and \verifier run $\pro{EvalB}(\crs,z,y,d,p-1;f(X))$\\
		$\pro{EvalB}(\crs,\gr{C}\in \GG,z\in \FF_p,y\in \FF_p,d\in \NN,b\in \ZZ;f(X)\in B_{b}[X])$
	    \begin{enumerate}[nolistsep]
        \item \pcif $d=0$:
...
\item \pcind[1] \verifier outputs $1$ \pcif all checks pass, $0$ otherwise.
         ...
         \item \pcind[1] \prover and \verifier run $\pro{EvalB}(\crs,C',z,y',d+1,b;f'(X))$
        ...
        \item \pcind[1] \prover and \verifier run $\pro{EvalB}(\params, \gr{C}', z, y', d',b' ; f'(X))$
               \end{enumerate}
      \end{minipage}
\end{mdframed}
