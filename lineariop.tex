...
and the verifier makes $\emph{linear queries}$ to $\pi$. The PCP is modeled as giving the verifier access to an oracle that receives queries of the form $\mathbf{q} \in \FF^n$ and returns the inner-product $\langle \proofO, \mathbf{q} \rangle$. 
