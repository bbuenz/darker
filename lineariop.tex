...
We show in the most essential terms how this Polynomial IOP is constructed, replacing Reed-Solomon proximity testing with polynomial commitment schemes. We omit many details refer to the \textsf{STARK} paper~\cite{C:BBHR19}, and to the references therein, for a complete treatment.

\paragraph{\textsf{STARK} Polynomial IOP} Let $g$ be the generator of a large enough subgroup of $\mathbb{F}$. The preprocessing consists of computing the unique polynomial $z(X) \in \mathbb{F}[X]$ of degree $T-2$ such that $z(g^1) = z(g^2) = \cdots = z(g^{T-1}) = 0$. The online proof starts with the prover computing the $\mathsf{w}$ polynomials $w_j(X)$ of degree at most $T-1$ such that $w[i,j] = w_j(g^i)$, and sending these to the verifier. The verifier responds with the random weight vector $\boldsymbol{\beta} \sample \mathbb{F}^k$. The prover computes $f(X) = \boldsymbol{\beta}^\mathsf{T} \boldsymbol{\mathcal{P}}(w_1(X), \ldots, w_\mathsf{w}(X), w_1(g \cdot X), \ldots, w_\mathsf{w}(g \cdot X))$. Note that for a valid witness, $f(g^1) = f(g^2) = \cdots = f(g^{T-1}) = 0$, so $z(X)$ divides $f(X)$. The prover sends $q(X) = f(X)/z(X)$ to the verifier. Note that $q(X)$ has degree at most $\mathsf{d}$. The verifier queries the following evaluations:
\begin{itemize}
    \item For all boundary constraints $(i, j, \alpha)$ he queries $s_j(g^i)$ and if the resulting value is different from $\alpha$ he aborts and rejects. This establishes that all $s_j(X)$ have degree at most $T-1$.
    \item For a random point $h \sample \mathbb{F}$, he queries $s_j(h)$ and $s_j(gh)$ for all $j \in [\mathsf{w}]$ as well as $q(h)$ and $z(h)$. This establishes that $q(X)$ is of degree at most $\mathsf{d}$. Then he tests whether $\boldsymbol{\beta}^\mathsf{T} \boldsymbol{\mathcal{P}}(s_1(h), \ldots, s_\mathsf{w}(h), w_1(gh), \ldots, w_\mathsf{w}(gh))$ equals $q(h) \cdot z(h)$ and if not he aborts and rejects.
\end{itemize}
