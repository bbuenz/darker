...
$p_1,\ldots,p_\ell: \FF^{\mu_i} \rightarrow \FF^{m_i}$ such that $\qgen(\textsf{st}^V_{i-1}, \mathit{coins}_i) = \qgen(i, \mathit{coins}_i) = p_i(\boldsymbol{\sigma_i})$. The IOP is \textbf{algebraic} %for a parameter $\lambda$
if $d = poly(\lambda)$.
...
A public-coin $(\mu, d)$ Polynomial IOP is a public-coin algebraic linear IOP where in all rounds $\ell=1$ and each query-response $\langle \boldsymbol{\pi}_i , \mathbf{q}_{i} \rangle$ corresponds to the evaluation of some polynomial $f \in \mathbb{F}[X_1, \ldots, X_\mu]$ in the seed vector $\boldsymbol{\sigma_i} \in \mathbb{F}^\mu$ from which $\mathbf{q}_i$ was derived.
\end{definition}
