...
\begin{proof}
By definition of a $(\mu, d)$ algebraic linear IOP, in each $i$th round of the IOP there are $\ell$ query generation functions $P_1,\ldots,P_\ell: \FF^\mu \rightarrow \FF^{m_i}$, where each $P_k$ is a vector whose $j$th component is a $\mu$-variate degree-$d$ polynomial $p_{k,j}$. There is a public query algorithm that first derives a seed $\boldsymbol{\sigma_i} \in \FF^\mu$ from the verifier's $i$th round public-coin challenge $\mathit{coins}$ and then computes $\mathbf{q}_k \gets P_k(\boldsymbol{\sigma_i})$ for all $k \in [\ell]$. The  generates generate the query matrix $\mathbf{Q} \in \mathbb{F}^{m_i \times \ell}$.
