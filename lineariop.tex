...
We note that the separation into two maps $\mathit{coins}_i \xmapsto{\qgen_0(i, \cdot)} \boldsymbol{\Sigma}_i \xmapsto{\qgen_1(i, \cdot)} \mathbf{Q}_i$ departs from the definition of Bitansky~\emph{et al.}, who instead require that $\mathbf{Q}_i$ be determined via $\vec{p}_1, \ldots, \vec{p}_\ell$ when evaluated in a random $\boldsymbol{r} \sample \mathbb{F}^{\mu_i}$. This departure constitutes a relaxation of the definition because the first map, $\qgen_0(i, \cdot)$, can simply lift the given string of random bits to the space $\mathbb{F}^{\mu_i}$ and output the $\ell$-fold repetition of that vector. The point is that $\qgen_0$ can also do other, non-algebraic computations. The separation into two steps is only meaningful when $\mu_i$ is much smaller than $m_i$, becaue this relative size difference captures the intuition whereby a computationally simple verifier outputs a short message, and outsources the expansion thereof into a list of large linear query vectors to some well defined algebraic circuit. Furthermore, this relaxation allows us to capture Polynomial IOPs as a special case of algebraic linear IOPs. We present a standalone definition of Polynomial IOPs, and follow up by demonstrating the inclusion.
...
The recent systems \pro{PLONK} and \pro{Marlin}\cite{Marlin} are improvements on the underlying Polynomial IOP in \pro{Sonic}, and achieves the following: 
...
\end{theorem}
\begin{theorem}[Marlin,~\cite{Marlin}] 
There exists a 4-round HVZK Polynomial IOP with preprocessing for any NP relation $\mathcal{R}$ (with arithmetic complexity $n$) that makes $3$ queries overall to univariate degree $6n$ polynomial oracles. The preprocessing verifier does $O(n)$ work to check $9$ univariate degree $n$ polynomials.
\end{theorem}
