...
$p_1,\ldots,p_\ell: \FF^{\mu_i} \rightarrow \FF^{m_i}$ such that $\qgen(\textsf{st}^V_{i-1}, \mathit{coins}_i) = p_i(\boldsymbol{\sigma_i})$. The IOP is \textbf{algebraic} %for a parameter $\lambda$
if $d = poly(\lambda)$.
