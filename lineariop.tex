...
\subsubsection{STARK}
The \textsf{STARK} proof system~\cite{C:BBHR19} builds a Reed-Solomon IOP of Proximity (RS-IOPP) for statements about uniform computations whose transition function can be expressed by an arithmetic circuit. The resulting IOPP can be compiled into a concrete, transparent proof system using FRI and Merkle trees.

Given the interpretation of FRI with Merkle trees as a polynomial commitment scheme in its own right, it should be obvious that \emph{any} RS-IOPP, including \textsf{STARK} in particular, is a special case of a Polynomial IOP compiled down with a polynomial commitment scheme.

\begin{theorem}[\cite{C:BBHR19}]
Let $\mathcal{L}$ be a language decidable in nondeterministic polynomial time $T(n)$ and let $\mathcal{R}$ be the relation induced by $\mathcal{L}$. There is a Polynomial IOP for $\mathcal{R}$ with $\frac{\log T(n)}{2} + O(1)$ rounds, perfect completeness, soundness error $2^-\lambda$ and knowledge error $O(2^{-\lambda})$, and perfect honest-verifier zero-knowledge. TODO finish me properly
\end{theorem}
