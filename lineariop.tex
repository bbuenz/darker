...
\noindent \underline{Protocol syntax}. 
For each $i$th round there is a prover state $\textsf{st}^\prover_i$ and a verifier state $\textsf{st}^\verifier_i$. For any common input $x$ and $\mathcal{R}$ witness $w$, at round 0 the states are $\textsf{st}^\prover_0 = (x, w)$ and $\textsf{st}^\verifier_0 = x$. 
In the $i$th round (starting at $i = 1$) the prover outputs a single proof oracle $\prover(\textsf{st}^\prover_{i-1}) \rightarrow \pi_i$, which is either a polynomial $\pi_i(\mathbf{X}) \in \mathbb{F}[\mathbf{X}]$ or empty $\pi_i = \varnothing$. The verifier computes a point list and index $\verifier(\st^\verifier_{i-1}) \rightarrow (\boldsymbol{\Sigma}_i, j_i)$ where $\boldsymbol{\Sigma}_i = (\boldsymbol{\sigma}_{i,1}, \ldots, \boldsymbol{\sigma}_{i, \ell}) \in \mathbb{F}^{\mu \times \ell}$ and $j_i \in [i-1]$ indicates a non-empty oracle. The verifier obtains the response vector associated with the $j_i$th oracle: $(\pi_{j_i}(\boldsymbol{\sigma}_{i,1}), \ldots, \pi_{j_i}(\boldsymbol{\sigma}_{i, \ell})) = \mathbf{a}_i \in \mathbb{F}^{1 \times \ell}$. The updated prover state is $\textsf{st}^\prover_i \leftarrow (\textsf{st}^\prover_{i-1}, \boldsymbol{\Sigma}_i, j_i)$
and verifier state is $\textsf{st}^\verifier_i \leftarrow (\textsf{st}^\verifier_{i-1}, \boldsymbol{\Sigma}_i, j_i, \mathbf{a}_i)$. Finally, $\verifier(\textsf{st}^\verifier_t)$ returns $1$ or $0$. \\ 
...
\subsubsection{STARK}
The \textsf{STARK} proof system~\cite{C:BBHR19} builds an IOP for uniform computations whose transition function can be expressed by an arithmetic circuit. The verifier in this IOP is tasked with determining the proximity of given proof oracles to certain Reed-Solomon codes. The IOP itself is then compiled into a concrete transparent proof system using FRI and Merkle trees.

Given the capability to expand FRI with Merkle trees into a polynomial commitment scheme, it follows that any abstract proof system that relies only on this primitive, can be cast a univariate Polynomial IOP. In particular, this formalism immediately captures the IOP underlying the \textsf{STARK} proof system as a special case. To illustrate this capture, we simplify the generic AIR language and rephrase the relevant compilation theorem in terms of a Polynomial IOP. Note that the original AIR is more complex and indeed gives way to the even more complex BAIR/PAIR/BPAIR languages for optimization, but still serves the illustrative purpose.

AIR represents a computation as an algebraic execution trace of a system of $\mathsf{w}$ registers taking values from a finite field $\mathbb{F}$, evolving for $T$ time steps according to a state transition function $\boldsymbol{\mathcal{P}}$. The trace itself is represented as an array of $\mathsf{w} \times T$ field elements. The state transition function has a representation in terms of polynomials in $2\mathsf{w}$ variables, the current and next state. In addition to that, certain boundary conditions are satisfied, meaning that specific registers take specific values at certain points.

\begin{definition}
The relation $\mathcal{R}_\mathsf{AIR}$ is the set of all instance-witness pairs $(x, w)$ satisfying the following description:

\noindent\underline{Instance.} An instance is a tuple $x = (\mathbb{F}, T, \mathsf{w}, \boldsymbol{\mathcal{P}}, \boldsymbol{\mathcal{B}})$ where
\begin{itemize}\itemsep0pt
    \item $\mathbb{F}$ is a finite field.
    \item $T \in \mathbb{N}$ is the number of time steps.
    \item $w$ is the number of registers.
    \item $\boldsymbol{\mathcal{P}} \in (\mathbb{F}[X_1, \ldots, X_\mathsf{w}, Y_1, \ldots, Y_\mathsf{w}])^k$ are $k$ polynomials of degree at most $\mathsf{d}$ called the state transition constraints.
    \item $\boldsymbol{\mathcal{B}} \in ([T] \times [\mathsf{w}] \times \mathbb{F})^\ell$ are $\ell$ tuples representing the boundary conditions.
\end{itemize}

\noindent\underline{Witness.} A witness is a table $w \in \mathbb{F}^{T \times \mathsf{w}}$ where each row $i \in [T]$ represents the full state of the system at time $i$, and each column $j \in [\mathsf{w}]$ tracks the value of register $j$ across time. A witness $w$ is a valid witness for $x$ iff and only if the following conditions are satisfied:
\begin{itemize}\itemsep0pt
    \item The state evolves consistently; in other words, the state transition constraints are satisfied for all consecutive pairs of states $(X_1, \ldots, X_\mathsf{w})$ and $(Y_1, \ldots, Y_\mathsf{w})$ up until time $T$. Symbolically: $\boldsymbol{\mathcal{P}}(w[i,1], \ldots, w[i,\mathsf{w}], w[i+1, 1], \ldots, w[i+1,\mathsf{w}]) = \boldsymbol{0}$ for all $i \in [T-1]$.
    \item The boundary conditions are satisfied; in other words, for every tuple $(i, j, \alpha) \in \boldsymbol{\mathcal{B}}$ we have $w[i,j] = \alpha$.
\end{itemize}

Furthermore, the language $\mathcal{L}_\mathsf{AIR}$ is the projection of $\mathcal{R}_\mathsf{AIR}$ onto the first coordinate, \emph{i.e.}, $\mathcal{L}_\mathsf{AIR} = \{x \, | \, \exists w \, (x,w) \in \mathcal{R}_\mathsf{AIR}\}$.
\end{definition}
...
We show in the most essential terms how this Polynomial IOP is constructed, replacing Reed-Solomon proximity testing with polynomial commitment schemes. We omit many details and refer to the \textsf{STARK} paper~\cite{C:BBHR19}, and to the references therein, for a complete treatment.

\paragraph{\textsf{STARK} Polynomial IOP} Let $g$ be the generator of a large enough subgroup of $\mathbb{F}$. The preprocessing consists of computing the nonzero polynomial $z(X) = \prod_{i=1}^{T-1} (X-g^i)$ which satisfies $z(g^1) = z(g^2) = \cdots = z(g^{T-1}) = 0$. The online proof starts with the prover computing the $\mathsf{w}$ polynomials $w_j(X)$ of degree at most $T-1$ such that $w[i,j] = w_j(g^i)$, and sending these to the verifier. The verifier responds with the random weight vector $\boldsymbol{\beta} \sample \mathbb{F}^k$. The prover computes $f(X) = \boldsymbol{\beta}^\mathsf{T} \boldsymbol{\mathcal{P}}(w_1(X), \ldots, w_\mathsf{w}(X), w_1(g \cdot X), \ldots, w_\mathsf{w}(g \cdot X))$. Note that for a valid witness, $f(g^1) = f(g^2) = \cdots = f(g^{T-1}) = 0$, so $z(X)$ divides $f(X)$. The prover sends $q(X) = f(X)/z(X)$ to the verifier. Note that $q(X)$ has degree at most $\mathsf{d}$. The verifier queries the following evaluations:
\begin{itemize}
    \item For all boundary constraints $(i, j, \alpha)$ he queries $w_j(g^i)$ and if the resulting value is different from $\alpha$ he aborts and rejects. This establishes that all $w_j(X)$ have degree at most $T-1$.
    \item For a random point $h \sample \mathbb{F}$, he queries $w_j(h)$ and $w_j(gh)$ for all $j \in [\mathsf{w}]$ as well as $q(h)$ and $z(h)$. This establishes that $q(X)$ is of degree at most $\mathsf{d}$. Then he tests whether $\boldsymbol{\beta}^\mathsf{T} \boldsymbol{\mathcal{P}}(w_1(h), \ldots, w_\mathsf{w}(h), w_1(gh), \ldots, w_\mathsf{w}(gh))$ equals $q(h) \cdot z(h)$ and if not he aborts and rejects.
\end{itemize}
The preprocessing phase can be omitted if $g$ spans a subgroup of order $T-1$. In this case $z(X) = X^{T-1} - 1$ and it can be evaluated by the verifier locally in essentially constant time.
