...
Given the capability to expand FRI with Merkle trees into a polynomial commitment scheme, it follows that any abstract proof system that relies only on this primitive, can be cast as a univariate Polynomial IOP. In particular, this formalism immediately captures the IOP underlying the \textsf{STARK} proof system as a special case. To illustrate this capture, we simplify the generic AIR language and rephrase the relevant compilation theorem in terms of a Polynomial IOP. Note that the original AIR is more complex and is even superseded more still complex BAIR/PAIR/BPAIR languages for optimization, but the present definition still serves the illustrative purpose.

AIR represents a computation as an algebraic execution trace of a system of $\mathsf{w}$ registers taking values from a finite field $\mathbb{F}$, evolving for $T$ time steps according to a state transition function $\boldsymbol{\mathcal{P}}$. The trace itself is represented as an array of $T \times \mathsf{w}$ field elements. The state transition function has a representation in terms of polynomials in $2\mathsf{w}$ variables, which represent the current and next state for one time step. In addition to that, certain boundary conditions are satisfied, meaning that specific registers take specific values at certain points.
