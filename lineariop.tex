...
It has \textbf{algebraic queries} if, additionally, for each $i \in [t]$, $\mathit{coins}_i$ defines a \emph{seed matrix} $\boldsymbol{\Sigma}_i \in \mathbb{F}^{\mu_i \times \ell}$ and the map $\boldsymbol{\Sigma}_i \xmapsto{\mathcal{Q}(i, \cdot)} \mathbf{Q}_i \in \mathbb{F}^{m_i \times \ell}$
...
Furthermore, a Polynomial IOP is \defn{stateless} if for each $i \in [t]$, $\mathcal{V}(\st^\verifier_{i-1}) = \mathcal{V}(i)$. 
