...
\item $\textsf{Extend}(i, h, \alpha_{i+1},...,\alpha_\mu)$ on $h \in \mathbb{Q}[X_1,\dots,X_{\mu-i}]$ returns $\bot$ if $h$ is not $\mu-i$ linear, and otherwise sets $h_i := h$ and runs the following iterative algorithm: for $j = i$ to $\mu-1$ set $h_{j+1} := h_{j, L} + \alpha_{j+1} \cdot h_{j,R}$ where, treating each $h_j$ as a degree ${\mu - j}$ linear polynomial (padding with zero coefficients), $h_{j,L}$ and $h_{j, R}$ are each ${\mu -j - 1}$ linear consisting of the left/right coefficients (i.e. the constant and linear part of $X_{\mu-j}$) of $h_j$, i.e. $h_j = h_{j,L} + X_{{\mu - j}}\cdot  h_{j,R}$; return $h_i,...,h_\mu$. \benedikt{double check the indices. I did but it's easy to be off by one}
