...
We extend the integer encoding from \Cref{sec:encoding} to rational multi-linear polynomials.
...
\begin{proof}
To see why this is binding over $\mathcal{M}(\beta_n, \beta_h)$, suppose that both $h=\frac{f}{N}$ and $h'=\frac{f'}{N'}$ are valid openings of a commitment $\gr{C}$ to distinct rational polynomials $h, h' \in \mathcal{M}(\beta_n, \beta_d)$. This implies that $N\cdot \gr{C} = f(\vec{q}) \cdot \gr{G}$ and $N' \cdot \gr{C} = f'(\vec{q}) \cdot \gr{G}$. Hence, $v =  N \cdot f'(\vec{q}) - N' \cdot f(\vec{q})$ is a multiple of the order of $\gr{G}$, i.e. $v \cdot \gr{G} = 0$. Moreover, by Lemma~\ref{lem:encoding}, $h(\vec{q}) \neq h'(\vec{q})$, which implies $v \neq 0$. This breaks the random order assumption for $\GG$. It is important to note that the openings $(f, N)$ and $(f', N')$ need not be co-prime elements of bounded norm; it suffices that $h = f/N$ and $h' = f'/N'$ have bounded norm numerators and denominators in reduced fraction form, i.e. $h, h' \in \mathcal{M}(\beta_n, \beta_d)$. 
\end{proof}
...
level label/.style={
  ...
  before drawing tree={
   ...
   [$13$ (root):\\ ${\instancecolor{\gr{C}\text{ \textcolor{black}{(input)}},\gr{C}_L,\gr{C}_R}: \gr{C}_L+q^{2}\cdot \gr{C}_R=\gr{C} }$\\ ${\witnesscolor{\hat{f}_L,\hat{f}_R,N_L,N_R}: \hat{f}/N=\hat{f}_L/N_L+X_2 \hat{f}_R/N_R}$\\
   ${ \hat{f}_L(q)\cdot \gr{G}=N_L\cdot \gr{C}_L,\hat{f}_R(q)\cdot \gr{G}=N_R\cdot \gr{C}_R }$\\ $
   {\phi_a(\hat{f},N ,\alpha_3^{(12)},\alpha_3^{(11)})=1}$, baseline,  
   [$4$:\\
    ${\instancecolor{\gr{C}_L,\gr{C}_R,\gr{C}}:\gr{C}=\gr{C}_L^{(13)}+\alpha_1 \gr{C}_R^{(13)}}$\\ 
    ${\witnesscolor{\hat{f},N}:\hat{f}(q)\gr{G}=N\cdot \gr{C}}$ ,my edge label={\alpha_1} 
    [$1$:\\ 
     ${\instancecolor{\gr{C}\in \GG}}$\\ ${\witnesscolor{\hat{f}\in \ZZ,\hat{f}<\CorrectnessBound{2}}:}$\\
     ${\hat{f}\cdot \gr{G}=\gr{C}}$, my edge label={\alpha_1}]
     [$2$:\\ ${\instancecolor{\gr{C}\in \GG}}$\\ ${\witnesscolor{\hat{f}\in \ZZ}}$, my edge label={\alpha_2}]
     [$3$:\\
         ${\instancecolor{\gr{C}\in \GG}}$\\ ${\witnesscolor{\hat{f}\in \ZZ}}$, my edge label={\alpha_3}]]
 [$8$:\\   ${\instancecolor{\gr{C}  \in \GG}}$\\ ${\witnesscolor{\hat{f} \in \ZZ[X],N \in \ZZ}}$, my edge label={\alpha_2}
,
 align=center
 [$5$:\\
