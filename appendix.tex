...
In round $i$ of the protocol the $\prover$ computes $f_L(X)$ and $f_R(X)$ such that $f_L(X_1,\dots,X_i)+X^{\frac{d+1}{2}} f_R(X)=f(X)$. Consequently $f(z_1,dots,) \bmod p=f_L(z)+ z^{\frac{d+1}{2}} f_R(z)\bmod p=y_L+z^{\frac{d+1}{2}}  y_R\bmod p =y$. The \textsf{PoE} protocol has perfect correctness so {$\gr{g}^{f_L(q)+q^{\frac{d+1}{2}} f_R(X)}=\gr{C}$}.
