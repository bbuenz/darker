...
Statement: $(\crs,\gr{C}_1,\gr{C}_2,z,y_1,y_2,b,d)\in \mathcal{R}_{\pro{JE}}$
        ...
        \item \verifier samples $\alpha \sample [0,2^\lambda)$ and sends it to \prover
			\item \prover and \verifier compute $\gr{C}'\gets  \gr{C}_1+\alpha \cdot\gr{C}_2$ and $y'\gets  y_1 + \alpha \cdot y_2 \bmod p$
			\item \prover computes $f'(X)\gets f_1(X) +\alpha  f_2(X)$
