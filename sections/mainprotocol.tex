...
\begin{fact}
	For any integer $z \in  (-\frac{q^{d+1}}{2}, \frac{q^{d+1}}{2})$ where $q$ is odd there exists a unique integer polynomial $f \in \ZZ(\frac{q-1}{2})[X]$ of degree at most $d$ such that $\pro{Enc}(f) = z$. Thus $z$ is a valid encoding of a unique polynomial in $\ZZ_p[X]$ equivalent to $f \bmod p$. 
\end{fact}
\begin{proof}
...
The encoding also has homomorphic properties. If $h(X) \leftarrow f(X) + g(X) \in \ZZ(b)[X]$ then $h(q) = f(q) + g(q)$, hence $\enc(g(X))+\enc(h(X))=\enc(g(X)+h(X))$. Additionally, if $h'(X) \leftarrow f(X) \cdot g(X) \in \ZZ(b)[X]$ then $\enc(g(X))\cdot \enc(h(X))=\enc(g(X)\cdot h(X))$.
