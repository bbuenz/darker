...
\item \textbf{Encoding.}
For any integer $q$, the function $\mathsf{Enc} : \mathbb{Z}(b)[X] \rightarrow \mathbb{Z}$ maps $h(X) \mapsto h(q)$. A polynomial $f(X) \in \ZZ_p[X]$ is first mapped to $\ZZ(p)[X]$ by replacing each coefficient of $f$ with its unique integer representative from $[0,p)$ of the same equivalence class modulo $p$.  %This means the map can be used as encoding of polynomials in the domain $\ZZ(b)[X]$ with $b<q/2$.
...
\begin{mdframed}
\begin{minipage}{\textwidth}
			$\pro{Eval}(\crs, \gr{C}\in \GG, z\in \ZZ_p, y\in \ZZ_p; f(X)\in \ZZ_p[X], \hat{f}(X) \in \ZZ[X]) :$  \pccomment{$f(X)=\sum_{i=0}^d f_i X^i$}
			\begin{enumerate}[nolistsep]
			\item Let $d=2^{\lceil \log_2(\deg(f)+1)\rceil}$
			\item \prover and \verifier run $\pro{EvalBounded}(\params,\gr{C},z,y,d,||\hat{f}||_\infty ;f(X),\hat{f}(X))$
		    \end{enumerate}
		    		\vspace{1em}
...
\benedikt{Remove section and replace with paragraph}
...
A careful analysis shows that if the coefficients of $f'(X)$ are bounded by $b$ then $f_L(X)$ and $f_R(X)$ must have coefficients bounded by $b \cdot p$ in absolute value\benedikt{Fix}. Using a similar analysis we can show that $f(z)\bmod p=y$ for the extracted polynomial $f(X)$.
