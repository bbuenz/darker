...
<<<<<<< HEAD (Ben's version) 
\begin{fact}
	$\enc: \ZZ(\frac{q-1}{2})[X] \rightarrow \ZZ(\frac{q^{d+1}-1}{2})$ is a bijection for odd $q$. Thus, any integer $z \in  \ZZ(\frac{q^{d+1}-1}{2})$ is a valid encoding of a unique polynomial in $\ZZ_p[X]$, recovered as $\pro{Dec}(z) \bmod p$.  
\end{fact}
\begin{proof}
For any $f \in \ZZ(\frac{q-1}{2})[X]$ of degree at most $d$, we see $\pro{Dec}(\pro{Enc})(f) = f$. This shows that $\pro{Enc}$ is injective. The cardinality of the domain is $q^{d+1}$ and is equal to the cardinality of the image, \emph{i.e.} $2 \cdot \frac{q^{d+1} - 1}{2} + 1 = q^{d+1}$. This shows that $\pro{Enc}$ is surjective. 
\end{proof}

The encoding also has homomorphic properties. If $h(X) \leftarrow f(X) + g(X)$ then $h(q) = f(q) + g(q)$, hence $\enc(g(X))+\enc(f(X))=\enc(h(X))$ assuming that $f, g, h \in \ZZ(b)[X]$. Additionally, if $h'(X) \leftarrow f(X) \cdot g(X) \in \ZZ(b)[X]$ then $\enc(g(X))\cdot \enc(h(X))=\enc(g(X)\cdot h(X))$.
