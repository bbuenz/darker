...
The FRI protocol~\cite{ICALP:BBHR18} is an efficient IOP that a committed oracle is close to a Reed-Solomon code word, i.e. a degree $d$ polynomial. FRI has been used in multiple recent zero-knowledge proof systems such as  \textsf{STARK}~\cite{C:BBHR19}, \textsf{Aurora}~\cite{EC:BCRSVW19}, and \textsf{Fractal}~\cite{Fractal}.
FRI and the improvement DEEP-FRI~\cite{DEEPFRI}. 

The FRI protocol commits to a polynomial in a subdomain and has many similarities to a polynomial commitment scheme. For full usage as a polynomial commitment the protocol needs to permit querying the polynomial outside of the committed domain. DEEP-FRI shows that this is possible and Vlasov and Panarin \cite{MatterLabs} make the connection explicit and build a polynomial commitment from FRI. The FRI-based commitment has proof size and verifier time $O(k\log^2(d))$ for a statistical security parameter $k$. It is not known how to use the commitment for multivariate polynomials. The commitment has intriguing properties as it only relies on symmetric key cryptography and is plausibly quantum resistant. 
\begin{comment}
