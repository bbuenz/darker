...
\paragraph{Hiding and Zero-knowledge.} 
We can use the general Cramer Damgard\cite{C:CraDam98} transform to turn the evaluation scheme into a 
Turning the polynomial commitment into a hiding scheme is easy by replacing $\gr{g}^{\bar{f}(q)}$ with a Pedersen commitment to $\bar{f}(q)$ over $\GG$. The setup parameters must contain two independent generators $\gr{g}, \gr{h}$ for which the discrete logarithm between $\gr{g}$ and $\gr{h}$ is unknown. %(Previously, in the non-hiding scheme, any generator would suffice).
...
Statement: $(\crs,\gr{C}_1,\gr{C}_2,z,y_1,y_2,b,d)\in \mathcal{R}_{\pro{JoinedEval}}$
			\begin{enumerate}[nolistsep]
