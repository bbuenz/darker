...
\begin{theorem}~\label{thm:polycommitsecurity} 
	\maintheorem
\end{theorem}
The proof of the theorem which closely follows the proof idea is in the \appendixphrase~(\ref{appendix:maintheoremproof}).
...
It has highly batchable proofs and it is possible to evaluate $n$ degree $d$ polynomials at $k$ points using only $2\log_2(d+1)$ group elements and $(k+1)\log_2(d+1)$ field elements (Appendix \ref{subsec:optimization}). Note that this means the proof size is independent of $n$ and linear in $k$ but with a small constant $(15 \log(d)$ bytes).
