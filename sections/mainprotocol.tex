...
In Table \ref{tab:polycommit} we give a comparison between different polynomial commitment schemes from literature. 
\begin{table}[!htp]
\begin{small}
\begin{tabular}{l||l|l|l|l|l}
	...
	\hline
	\hline
    DARK  (This work) & yes & $O(1)$ & $O( d^\mu \mu \log(d) )\; \textsf{EXP}$ & $2\mu \log(d)~\textsf{EXP}$ & $2 \mu \log(d) \GG_{U}$ \\
    Pairing \cite{AC:KatZavGol10} & no & $d^\mu$ $\GG_{B}$ & $O(d^\mu)\; \textsf{EXP}$  & $\mu\; \textsf{Pairing} $ & $\mu\; \GG_{B}$\\
    D-Log \cite{EC:BCCGP16} & yes & $\sqrt{d^\mu}\GG_{P}$ & $O(d^\mu)$ \textsf{EXP} & $O(\sqrt{d^\mu})\textsf{EXP}$ &$O(\sqrt{d^\mu}) \; \GG_P$\\
       BP \cite{EC:BCCGP16,SP:BBBPWM18} & yes &$2 d^\mu\GG_{P}$& $O(d^\mu)$ \textsf{EXP}& $O(d^\mu)\textsf{EXP}$ &$2 \mu \log(d) ; \GG_P$\\
       FRI\cite{DEEPFRI,MatterLabs} (univar.) & yes & $O(1)$ & $O(\lambda  d)$ $\hash$ & $O(\lambda \log^2(d))$ $\hash$ & $O(\lambda \log^2(d))$ $\hash$
\end{tabular}

\caption{Comparison table between different polynomial commitment schemes for an $\mu$-variate polynomial with $d$ coefficients (degree $d-1$). 
In particular we evaluate the reference string size, the prover and verifier time as well as the evaluation proof size. Column $2$ indicates whether the setup is transparent, i.e. whether the CRS is structured.
