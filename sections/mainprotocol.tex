...
The coefficients of $f$ can be recovered via the base $q$ decomposition of $f(q)$. For conveniece, let $\ZZ(b):=\{x \in \ZZ: \vert x \vert  \leq b\}$ denote the set of integers with absolute value less than or equal to $b$.
This encoding is an injective mapping from polynomials in $\ZZ(q-1)[X]$ of degree at most $d$ to the set $[0, q^{d+1})$. The encoding is also \emph{partially} homomorphic. 
