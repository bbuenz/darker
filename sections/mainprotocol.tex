...
The prover could then commit to $g^*_L(q)$ and $g^*_R(q)$, and recurse on $g^*_L(q) + \alpha  g^*_R(q)$ instead of $ f^*_L(q) + \alpha f^*_R(q)$. This would be non-binding. (For example $f^*(X)= q-1$ and $g^*(X)=X-1$, or $f^*(X) = q +1$ and $g^*(X) = X + 1$). 

\paragraph{Inferring coefficient bounds}\benedikt{not wrong but not informative} So what can the verifier infer from the opened commitment $[\![f']\!]$ at the bottom level of recursion? The opened commitment is an integer $f' =f_L +  \alpha f_R$. 
...
As shown above, the verifier can infer a bound on the absolute values of $f_L$ and $f_R$, but still cannot infer that $f_L$ and $f_R$ are both \emph{positive} integers. Moreover, if $f_R > 0$ and $f_L < 0$, then it is still possible that $f_L + q f_R > 0$, and thus that there is a distinct $g \neq f$ with $q$-bounded positive coefficients such that $g(q) = f(q)$. For example, say $f_R = q/2$ and $f_L = -1$ then $f_L + q f_R = q^2/2 - 1$, and $ f_L + \alpha f_R  = q/2 - \alpha > 0$ for every $\alpha \in [0,2^{\lambda})$. Yet, also $q^2/2 - 1 = g(q)$ for $g(X) = (q/2 - 1)X + q-1$. \benedikt{Check this}

It turns out that we also can't ensure that $f_L$ and $f_R$ but only that they are bounded rational polynomials. We show that the same encoding works even for rational polynomials in \Cref{appendix:prelimns}.

\paragraph{Ensuring injectivity} How can we ensure the encoding scheme is injective over polynomials with either positive/negative coefficients bounded in absolute value? Fortunately, it is a fact that if $|f_L| < q/2$ and $|f_R| < q/2$ then at least one coefficient of $g$ must be larger than $q/2$. In other words, if the prover had committed instead to $f^*_L$ and $f^*_R$ such that $g(X) = f^*_L + Xf^*_R$ then the verifier could reject the opening of $ \hat{f}^*_L + \alpha \hat{f}^*_R$ with overwhelming probability based on its size. 
...
\begin{mdframed}
\begin{minipage}{\textwidth}
			$\pro{Eval}(\crs, \gr{C}\in \GG, \vec{z}\in \ZZ_p^{\mu}, y\in \ZZ_p; f(X_1,\dots,X_\mu)\in \ZZ_p[X_1,\dots,X_\mu],) :$  \pccomment{$f$ is $\mu$-linear}
			\begin{enumerate}[nolistsep]
			\item \prover computes $\hat{f}(X_1,\dots,X_\mu) \in \ZZ(p)[X_1,\dots,X_\mu]$ such that $\hat{f} \bmod p \equiv f$
			\item \prover and \verifier run $\pro{EvalB}(\params,\gr{C},\vec{z},y,\mu,p-1;f,\hat{f})$
		    \end{enumerate}
		    		\vspace{1em}
		$\pro{EvalB}(\crs,\gr{C}\in \GG,\vec{z}\in \ZZ_p^{\mu},y\in \ZZ_p,\mu \in \NN,b\in \ZZ;f\in\ZZ_p[X_1,\dots,X_\mu],\hat{f}\in \ZZ(b)[X_1,\dots,X_\mu])$		
	    \begin{enumerate}[nolistsep]
        \item \pcif $\mu=0$:
        \item \label{line:basestart}\pcind[1] \prover sends $\hat{f}\in \ZZ$ to the verifier. \pccomment{$\hat{f}$ is a constant}
...
\item \pcind[1] \verifier outputs $1$ \pcif all checks pass, $0$ otherwise.
        ...
        \item \pcind[1] \prover and \verifier compute $\mu' \gets \mu-1$
        \item \pcind[1] \prover computes $\hat{f}_L(X_1,\dots,X_\mu')$ and $\hat{f}_R(X_1,\dots,X_\mu')$ such that $\hat{f}=\hat{f}_L + X_\mu \hat{f}_R$
        \item \pcind[1] \prover computes $f_L,f_R$ analogously for $f$
        \item \pcind[1] \prover computes $y_L\gets f_L(z_1,\dots,z_\mu') \bmod p$ and $y_R\gets f_R(z_1,\dots,z_\mu')\bmod p$
        \item \pcind[1] \prover computes $\gr{C}_L \gets \hat{f}_L(q,q^2,\dots,q^{(2^{\mu'-1})})\cdot  \gr{g}$ and $\gr{C}_R \gets \hat{f}_R(q,q^2,\dots,q^{(2^{\mu'-1})})\cdot \gr{g}$
        ...
        \item \pcind[1] \verifier checks that $y=y_L+z_\mu \cdot y_R \bmod p$, outputs $0$ if check fails.
        \item \pcind[1]\label{line:PoE} \prover and \verifier run $\pro{PoE}(\gr{C}_R, \gr{C}-\gr{C}_L, q^{(2^{\mu'})})$\pccomment{Showing that $\gr{C}_L+q^{(2^{\mu'})}\cdot  \gr{C}_R=\gr{C}$}
        \item \pcind[1] \verifier samples $\alpha \sample [0,2^\lambda)$ and sends it to \prover
        \item \pcind[1] \prover and \verifier compute $y'\gets  y_L +\alpha \cdot y_R \bmod p$, $\gr{C}' \gets  \gr{C}_L + \alpha\cdot \gr{C}_R$, $b'\gets b \cdot 2^\lambda$. 
      \item \pcind[1]  \prover computes $f' \gets  f_L + \alpha\cdot f_R \in \ZZ_p[X_1,\dots,X_{\mu'}]$ 

        \item \pcind[1] \prover computes $\hat{f}' \gets \hat{f}_L + \alpha\cdot \hat{f}_R \in \ZZ[X_1,\dots,X_{\mu'}]$ 
        \pccomment{$\hat{f'}$ and $f'$ are $\mu'$-linear}
        \item \pcind[1] \prover and \verifier run $\pro{EvalB}(\params, \gr{C}', \vec{z}'=(z_1,\dots,z_\mu'), y', \mu',b' ;f', \hat{f}')$
               \end{enumerate}
      \end{minipage}
\end{mdframed}
%\end{small}
...
The polynomial commitment scheme is correct for $\mu$-linear polynomials in $\ZZ_p[X]$.
}
...
The proof of this lemma is in Appendix~\ref{sec:darksoundness}.
...
\newcommand{\maintheorem}{
	Let $\CSZ = 8 \mu^{2} + \log_2 (2 \mu) \lambda$. Let $\EBL=\lambda\cdot \mu$ and $\CorrectnessBound=\lambda\cdot \mu+\log_2 p$. Let $\com$ be the DARK commitment scheme as described in \Cref{lem:darkcommit}. There exists a pair of predicates $\phi$ such that the $\mu$-round DARK polynomial commitment evaluation protocol $\eval'$ with $\lambda$-bit challenges, a group of unknown order $\ggen$, and $\log q \geq 4(\lambda + 1 + \CSZ[\mu]) + \EBL[\mu] + \CorrectnessBound + 1$ is $(2^{(\mu)}, \frac{3\mu}{2^\lambda},\com,\phi)$-almost-special-sound .\footnote{The $\CSZ$ value can be replaced with values from the table in \Cref{lem:cCSZ}} 
...
We describe the performance of our scheme for different settings for uni- and multivariate instantiations in Table~\ref{tab:performance}.
