...
\item \pcind[1] \verifier outputs $1$ \pcif all checks pass, $0$ otherwise.
        ...
        \item \pcind[1] \prover sends $y_L,y_R, \gr{C}_L, \gr{C}_R$ to \verifier. \pccomment{See full version for an optimization}
...
All security proofs are in the full version of this paper~\cite[\S A.1 -- \S A.2]{EPRINT:BunFisSze19}.
...
\dyadicmaintheorem
\end{theorem}
The proof of Theorem~\ref{thm:dyadicpolysecurity} is nearly identical to the proof of Theorem~\ref{thm:polycommitsecurity} but the extracted polynomials are polynomials over the dyadic rationals and not over the integers. This requires the bound on $q$ to be larger by a factor of $p^{\log(d+1)}$. Both proofs are presented in the full version of this paper~\cite[\S A.3 -- \S A.4]{EPRINT:BunFisSze19}.

\subsection{Optimizations and Extensions}
Out of space constraints, a number of interesting but non-essential sections are omitted.
The full version of this paper~\cite{EPRINT:BunFisSze19} presents a range of optimizations for greater prover and verifier efficiency and smaller proof size. It also shows how to achieve extend the commitment to multivariate polynomials and shows how to make the commitment hiding with a ZK evaluation protocol. 
%Furthermore, the full version includes a comparison to three alternative strategies (and specific instances of these strategies) for generating polynomial commitment schemes, namely based on bilinear maps, based on discrete logarithms, and based on Merkle trees of Reed-Solomon codewords.
...
\begin{tabular}{l||l|l|l|l|l}
    ...
    \cite[$\sqrt{\cdot}$]{EC:BCCGP16} & yes & $\sqrt{d^\mu}\GG_{P}$ & $O(d^\mu)$ \textsf{EXP} & $O(\sqrt{d^\mu})\textsf{EXP}$ &$O(\sqrt{d^\mu}) \; \GG_P$\\
       ...
       FRI  ($\mu = 1$)\cite{EPRINT:ZXZS19}& yes & $O(1)$ & $O(\lambda  d)$ $\hash$ & $O(\lambda \log^2(d))$ $\hash$ & $O(\lambda \log^2(d)) \; \hash$
\end{tabular}
