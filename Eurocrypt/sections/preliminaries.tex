...
The \emph{$r$-Strong RSA Assumption} states that an efficient adversary cannot compute $\ell$th roots for a given random group element, if $\ell$ not a power of $r$. Specifically, it holds for $\ggen$ if for any probabilistic polynomial time adversary $\adv$:
\begin{small}
\[
