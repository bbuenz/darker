...
Even when precise factors are given the numbers should be seen as  estimates. For example, we chose to not display smaller order terms. The symbol $\GG_U$ denotes an element in group of unknown order, $\GG_{B}$ one in a group with a bilinear map (pairing), $\GG_{P}$ one in a prime order group with known order. Furthermore, $\textsf{EXP}$ refers to exponentiation of $\lambda$-bit numbers in these groups, and $\hash$ is either the size of a hash output or the time it takes to compute a hash. 
