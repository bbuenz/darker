...
In this section we show that one can construct any algebraic linear IOP from a (multivariate) Polynomial IOP. This construction rests on two tools for univariate Polynomial IOPs. These tools are treated explicitly in the full version of this paper~\cite{EPRINT:BunFisSze19}. They can be realized with a small constant number of evaluations.
\begin{itemize}
...
We formally prove Theorem~\ref{thm:algebraicIOPcompiler} in the full version of this paper~\cite{EPRINT:BunFisSze19}. Here we present the transformation without proof.
...
%The proof is provided in the full version of this paper~\cite[\S B]{EPRINT:BunFisSze19}. 
...
Several proof systems use Polynomial IOPs and our compiler can be applied to them. We present \textsf{PLONK}~\cite{Plonk} here and discuss several other proof systems \cite{Sonic,Marlin,C:BBHR19,Spartan,EC:GGPR13} in the full version~\cite{EPRINT:BunFisSze19}

Theorem~\ref{thm:supersonic} provides the main theoretical result of this work, tying together the new DARK polynomial commitment scheme (Theorem~\ref{thm:polycommitsecurity}), the compilation of Polynomial IOPs into SNARKs with preprocessing using polynomial commitments (Theorem~\ref{thm:IOPcompiler}), and a concrete univariate Polynomial IOPs. To enable this tie-up, we re-characterize the result of \textsf{PLONK} in terms of Polynomial IOPs, making use of the coefficient query technique (Section~\ref{sec:polynomialIOPreductions}) as necessary.

\begin{theorem}[\textsf{PLONK},~\cite{Plonk}] \label{thm:plonk}
...
Combining the \pro{PLONK} Polynomial IOP with the new transparent polynomial compiler of Section~\ref{sec:protocol} gives the following result. Analogous results are obtained by using \textsf{Sonic}~\cite{Sonic} or \textsf{Marlin}~\cite{Marlin} instead.  
...
%The proof is deferred to the full version of this paper~\cite{EPRINT:BunFisSze19}.
