...
In this section we show that one can construct any algebraic linear IOP from a (multivariate) Polynomial IOP. This construction rests on two tools for univariate Polynomial IOPs. These tools are treated explicitly in the full version of this paper~\cite{DARK/Supersonic:fullversion}; for now it suffices to say that these subprotocols are realizable with small number (constant) of evaluations.
\begin{itemize}
...
We consider here examples of Polynomial IOPs to which this compiler can be applied: \textsf{Sonic}~\cite{Sonic}, \textsf{PLONK}~\cite{Plonk} and \textsf{Marlin}~\cite{Marlin}. The full version of this paper~\cite{DARK/Supersonic:fullversion} extends this application to \textsf{STARK}~\cite{C:BBHR19}, \textsf{Spartan}~\cite{Spartan}, and the popular QAP of Gennaro~\emph{et al.}~\cite{EC:GGPR13}.
...
There is a 3-round HVZK Polynomial IOP with preprocessing for any NP relation $\mathcal{R}$ that makes $12$ queries overall to $12$ univariate degree $n$ polynomial oracles. The total number of distinct query points is $2$. The preprocessing verifier does $O(n)$ work to check $7$ of the univariate degree $n$ polynomials. Here, $n$ is the total number of arithmetic gates in the circuit that defines $\mathcal{R}$.
\end{theorem}

\begin{theorem}[\textsf{Marlin},~\cite{Marlin}] 
There exists a 4-round HVZK Polynomial IOP with preprocessing for any NP relation $\mathcal{R}$ that makes $20$ queries at $3$ distinct query points to $19$ univariate degree polynomial oracles of maximum degree  $6n$. The preprocessing verifier does $O(n)$ work to check $9$ univariate degree $n$ polynomials. Here, $n$ is the number of nonzero entries in the R1CS description of $\mathcal{R}$.
\end{theorem}
...
Despite its triviality, the proof is deferred to the full version of this paper~\cite{DARK/Supersonic:fullversion}.
