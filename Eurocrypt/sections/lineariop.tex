...
In this section we show that one can construct any algebraic linear IOP from a (multivariate) Polynomial IOP. This construction rests on two tools for univariate Polynomial IOPs. These tools are treated explicitly in the full version of this paper~\cite{EPRINT:BunFisSze19}; These subprotocols are realizable with small constant number of evaluations.
\begin{itemize}
...
Several proof systems use Polynomial IOPs and our compiler can be applied to them. We present \textsf{Sonic}~\cite{Sonic} here and discuss several other proof systems \cite{Plonk,Marlin,C:BBHR19,Spartan,EC:GGPR13} in the full version~\cite{EPRINT:BunFisSze19}
\if 0
