...
\begin{enumerate}[nolistsep]
	...
	\item \pcind $\alpha, \beta \gets \mathsf{H}((\mathsf{C}_O^{(0)}, \mathsf{C}_O^{\prime(0)}, \ldots, \mathsf{C}_O^{(\log_2(d)-1)}, \mathsf{C}_O^{\prime(\log_2(d)-1)}) \Vert \pi_{\mathsf{PoE}})$ \pccomment{Fiat-Shamir}
