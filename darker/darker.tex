...
We will show that given this tree, the extractor can compute for all nodes $v \in [0, d^{2})$ an opening hint $(\hat{f}_v \in \ZZ(b)[X],\denom_v \in \ZZ,\gr{W}_v \in \GG,a_v \in \NN\setminus \{0\})$ for $\gr{C}_v$.   That is $\gr{C}_v^\denom=\Generator^{\hat{f}_v(q)}+\gr{W}_v$ and $\gr{C}_v^{a_v}=0$ for $a_v>0$. At level $i$, $||\hat{f}||_\infty \leq b\cdot 2^{(\log_2(d)+2(\logd-i)) \cdot \lambda}$
...
This gives us the opening eqality
$$\hat{f}_{v_i}(q) \cdot \Generator + \gr{W}_i = \denom_i \cdot \Commitment_{E,w} + \denom_i \cdot \alpha_i \cdot \Commitment_{O,w} \quad i \in \{1,2\}$$
