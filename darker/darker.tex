...
Let $\gr{D}_{i}\gets \gr{C}_{O,i}$ and $\gr{D}_i'\gets \gr{C}_{i}-\gr{C}_{E,i}$ for $i \in [1,\logd]$ such that $q^{2^{\logd-i}} \cdot \gr{D}_i=\gr{D}'_i$ for all $i$. The prover and verifier run the following \textsf{BatchPoE} protocol that the claims hold.
\begin{mdframed}
Prover and Verifier Input: $[\gr{D}_i,\gr{D}_i']_{i=1}^{\logd},q$\\
Claim: $q^{2^{\logd-i}}\cdot \gr{D}_i=\gr{D}'_i \forall i \in [\logd]$
	\begin{enumerate}[nolistsep]
\item $\gr{X}_{\logd}=\gr{D}_{\logd}$, $\gr{Z}_{\logd}=\gr{D}'_{\logd}$
\item $\gr{Y}_{\logd}\gets q^{d/4}\cdot \gr{X}_{\logd}$ \pccomment{Computable as $ f_{O,\log_2(d)}\cdot \Generator_{\frac{d}{4}}^{}$ for $\Generator_{\frac{d}{4}}=q^{\frac{d}{4}}\cdot \Generator$}
\item \prover sends $\gr{Y}_{\logd}$ to $\verifier$
	\item $\pcfor i \in [\logd-1,2]$
	\item \pcind[1]$\verifier$ samples $\alpha,\beta \sample [-2^{\lambda-1} ,2^{\lambda-1}]$ and sends them to $\prover$
	\item \pcind[1]\prover and \verifier compute  $\gr{X}_{i}\gets  \gr{X}_{i+1}+ \alpha\cdot  \gr{Y}_{i+1}+\beta \cdot\gr{D}_{i}$
	\item $\pcind[1]$\verifier compute   $\gr{Z}_{i}\gets \gr{Y}_{i+1}+\alpha\cdot \gr{Z}_{i+1}+\beta\cdot \gr{D}_{i}^{'}$
		\item \pcind[1]\prover computes $\gr{Y}_{i}\gets q^{2^{i-2}}\cdot\gr{X}_{i}$ \pccomment{See discussion below}
		\item \pcind[1]$\prover$ sends $\gr{Y}_{i}$ to \verifier.
%\item \verifier samples random $\alpha,\beta \sample [-2^{\lambda-1} ,2^{\lambda-1}]$ and checks that $(\gr{X}_{2} \gr{Y}_{2}^{\alpha}\gr{D}_{1}^{\beta})^q=\gr{Y}_{2}\gr{Z}_{2}^{\alpha}\gr{D}_{1}^{'\beta}$
\item \verifier checks that $q\cdot \gr{X}_2=\gr{Y}_2$, $q\cdot \gr{Y}_2=\gr{Z}_2$ and $q\cdot \gr{D}_1=\gr{D}_1'$ (These checks can be combined using another pair of challenges).
\end{enumerate}
\end{mdframed}
...
\label{lem:aug-com-binding}
	The commitment is binding under the RSA assumption. 
\end{lemma}
\begin{proof}
	...
	\begin{lemma}[PoEs]
		The batch PoE protocol on input $[\gr{D}_i,\gr{D}_i']_{i=1}^{\logd}$ is an interactive proof of knowledge for the following relation:
		$$\mathcal{R}_{POE}=\{([\gr{D}_i,\gr{D}_i']_{i=1}^{\logd},q;\gr{W}_i\in \GG,a_i\in \ZZ): [\gr{D}_i'=q^{2^{\logd-i}}  \cdot \gr{D}_i+\gr{W}_i \wedge a_i\cdot \gr{W}_i=0]_{i=1}^{\logd-1} \}$$
	\end{lemma}
	\begin{proof}
