...
\end{lemma}
\begin{proof}
Suppose a polynomial time adversary is able to compute a commitment $\gr{C}$ and two polynomials $\hat{f}_0(X) \neq \hat{f}_1(X) \in \mathbb{Z}[X]$ with coefficients in $\mathbb{Z}(q/2)$ and such that $\hat{f}_0(q) \cdot \mathsf{G} = \hat{f}_1(q) \cdot \mathsf{G} = \mathsf{C}$. Then $\hat{f}_0(q) - \hat{f}_1(q) \neq 0$ is a multiple of the order of $\mathsf{G}$.
\end{proof}
...
\begin{proof}
We describe the operation of the PPT extractor machine \extractor, which has rewinding access to a possibly malicious prover $\prover^*$, which in turn succeeds with overwhelming probability in convincing the verifier. Consider the binary tree of depth $\lfloor \log d \rfloor$ where every node is labelled with a message from the prover and every edge is labelled with a message from the verifier. Every path from root to leaf corresponds to a transcript. \extractor dynamically generates this tree and computes a polynomial $\hat{f}^{(i)}(X)$ for every node such that $\hat{f}(q) \cdot \mathsf{G} = \mathsf{C}^{(i)}$ where $\mathsf{C}^{(i)}$ corresponds to the value of the variable $\mathsf{C}$ of the verifier in round $i$.

\extractor\, computes $\hat{f}^{(\lfloor \log_2 d \rfloor)}(X)$ by reading the corresponding leaf. For every non-leaf node, \extractor computes $\hat{f}^{(i)}(X)$ from the polynomials $\hat{f}^{(i+1)}(X)$ and $\hat{f}^{\prime (i+1)}(X)$, corresponding to the node's two children. Let $\alpha^{(i)}$ and $\alpha^{\prime (i)}$ be the verifier's challenges on the edges that connect the node to its children. Let $f_O = \frac{\hat{f}^{(i+1)}(q) - \hat{f}^{\prime (i+1)}(q)}{\alpha^{(i)} - \alpha^{\prime (i)}}$ such that $(\alpha^{(i)} - \alpha^{\prime (i)}) f_O \cdot \mathsf{G} = \mathsf{C}_O^{(i+1)} - \mathsf{C}_O^{\prime (i+1)}$.
