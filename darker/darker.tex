...
\begin{enumerate}[nolistsep]
	...
	\item \pcind \pcind $\mathsf{P}_i \gets \mathsf{P}_i + (\hat{h_j} + q^{j - \lfloor j / b \rfloor \cdot b}) \cdot \mathsf{G}_{\lfloor j / b \rfloor}$ 
...
\end{enumerate}
$\mathsf{EvalProve}(\mathsf{C}, \mathbf{z}, \mathbf{y}; f(X), (f_0(X), \ldots, f_{b-1}(X)), \left(\mathsf{P}_0, \ldots, \mathsf{P}_{b-1}\right))$ \textbf{:}
\begin{enumerate}[nolistsep]
	...
	\item \pcind $\mathbf{y}_O \gets \hat{f}_O(\mathbf{z})$
	\item \pcind \textbf{if} $i < \log_2 b$ \textbf{:}
	\item \pcind \pcind $\mathsf{C}_O \gets \mathsf{P}_{b-1}$
	\item \pcind \pcind \textbf{for} $j$ \textbf{from} $0$ \textbf{to} $b/2-1$ \textbf{:}
	\item \pcind \pcind \pcind $\mathsf{C}_O \gets \mathsf{P}_{b-1-2j} + q^{2^i} \cdot \mathsf{C}_O$
	\item \pcind \pcind $\mathsf{C}_O' \gets q \cdot \mathsf{C_O}$
	\item \pcind \textbf{else:}
	\item \pcind \pcind $\mathsf{C}_O \gets 0$ and $\mathsf{C}_O' \gets 0$
	\item \pcind \pcind \textbf{for all} nonzero terms $\hat{h}_jX^j$ in $\hat{f}_O(X^{2^i})$ \textbf{:}
	\item \pcind \pcind \pcind $\mathsf{C}_O \gets \mathsf{C}_O + \hat{h}_j \cdot \mathsf{G}_{\lfloor j/b \rfloor}$
	\item \pcind \pcind \pcind $\mathsf{C}_O' \gets \mathsf{C}_O' + \hat{h}_j \cdot \mathsf{G}_{\lfloor (j+2^i)/b\rfloor}$
	\item \pcind $\pi \gets \pi \Vert \mathbf{y}_O \Vert \mathsf{C}_O \Vert \mathsf{C}_O'$
