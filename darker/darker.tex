...
\item $\verify(\crs, f \in \ZZ_p[X], \Commitment,d \in \ZZ , \gr{W}\in \GG, \alpha \in \ZZ) \rightarrow b \in \{0,1\}$: 
The verification algorithm checks an opening of $\Commitment$ to the polynomial $f \in \ZZ_p$, given the opening hint $(\gr{W}, \alpha)$. It checks that $\alpha\cdot \gr{W}=0$, computes $f'(X)=f(X)\cdot d^{-1} \in \ZZ_p[X]$ and that $\Commitment^d =f'(q)\cdot \Generator +\gr{W}$
