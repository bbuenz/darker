...
\item $\mathsf{BatchPoE.Prove}$ has $\lfloor \log_2 d \rfloor$ iterations with $i \in \{1, \ldots, \lfloor \log_2 d \rfloor\}$. In each iteration there is one exponent of $q^{2^i}$ bits (line~\ref{line:batchpoeverify-q}) and four exponentiations of $\lambda$ bits (lines~\ref{line:batchpoeverify-regular} and~\ref{line:batchpoeverify-prime}). In total this amounts to $2d + \lfloor \log_2 q \rfloor + 1 + 4\cdot\log_2(d+1)\cdots\lambda$ bits !?!
\item $\mathsf{BatchPoE.Verify}$ has $\lfloor \log_2 d \rfloor$ iterations and in every iteration there are four $\lambda$-bit exponentiations. Finally, there is a test involving $1+\lfloor \log_2 q \rfloor$ exponent bits.
