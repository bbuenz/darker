...
\end{proof}
\begin{fact}
Let q be an integer. For any rational $z$ with numerator $n \in ZZ$ such that $|z|<\frac{q^{d+1}}{2}$ there exists a unique degree (at most) d rational polynomial $\hat{h}(X)$ in $\QQ(q)[X]$ such that $h(q) = z$. If $q>p$ then there exists a unique polynomial $h(X) \in \ZZ_p$ such that $\hat{h}\equiv h \bmod p$ \benedikt{$p$ doesn't really have to be prime.}
\end{fact}
...
\end{lemma}
\begin{proof}
	Given a commitment $\gr{C}$ and two openings $f(X),\tau=(\hat{f}(X)\in \ZZ(b)[X],d\in \ZZ,\gr{W},\alpha)$ and $f'(X),\tau'=(\hat{f}',d',\gr{W}',\alpha')$ with $\hat{f}(X)= d^{-1}\cdot  f(X)\neq d'^{-1} \cdot f'(X)=\hat{f}'(X)$. $\hat{f}$ and $\hat{f}'(X)$ have coefficients bounded by $\frac{q}{2}$ such that $\hat{f}(q)/d \neq f'(q)/d \in \QQ$. 
