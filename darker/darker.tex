...
[todo: line about sparse polynomials arising in natural computations; PCSs therefore should accomodate]
...
\begin{enumerate}[nolistsep]
	\item $\mathsf{C}_O^{(0)} \Vert \mathsf{C}_O^{\prime(0)} \Vert \mathbf{y}_O^{(0)} \Vert \cdots \Vert \mathsf{C}_O^{(\log_2(d) - 1)} \Vert \mathsf{C}_O^{\prime(\lfloor\log_2 d\rfloor)} \Vert \mathbf{y}_O^{(\lfloor\log_2 d\rfloor)} \Vert \pi_{\mathsf{PoE}} \Vert \hat{f} \gets \pi_{\mathsf{Eval}}$
	...
	\item \pcind $\alpha \gets \mathsf{H}(\tau)$ \pccomment{Fiat-Shamir}
...
\begin{enumerate}[nolistsep]
	...
	\item \pcind $\alpha, \beta \gets \mathsf{H}((\mathsf{C}_O^{(0)}, \mathsf{C}_O^{\prime(0)}, \ldots, \mathsf{C}_O^{(\log_2(d)-1)}, \mathsf{C}_O^{\prime(\log_2(d)-1)}) \Vert \pi)$ \pccomment{Fiat-Shamir}
...
\begin{enumerate}[nolistsep]
	\item $\mathsf{Y}^{(\lfloor\log_2 d\rfloor)} \Vert \cdots \Vert \mathsf{Y}^{(1)} \gets \pi_{\mathsf{PoE}}$
	...
	\item \pcind $\alpha, \beta \gets \mathsf{H}((\mathsf{C}_O^{(0)}, \mathsf{C}_O^{\prime(0)}, \ldots, \mathsf{C}_O^{(\lfloor\log_2 d\rfloor)}, \mathsf{C}_O^{\prime(\lfloor\log_2 d\rfloor)}) \Vert \pi_{\mathsf{PoE}})$ \pccomment{Fiat-Shamir}
...
When $i \in \{\log_2 b, \ldots, \lfloor \log_2 d \rfloor\}$ there are at most $s$ exponentiations of $i \cdot \lambda + 1 + \lfloor \log_2 p \rfloor$ bits (line~\ref{line:batchpoeprove-big}), or $s \cdot (\lfloor \log_2 d \rfloor - \log_2(b) + 1) \cdot (1 + \lfloor \log_2 p \rfloor) + s \cdot \lambda \cdot (\lfloor \log_2 d \rfloor + \log_2 b) \cdot (\lfloor \log_2 d \rfloor - \log_2(b) + 1)/2$ in total. (In fact, there are only at most $\max(s, \, 2^{\lfloor \log_2 d \rfloor - i})$ such exponentiations, but we take the easy upper bound $s$ for formulaic convenience.) \\
In each of these $\lfloor \log_2 d \rfloor$ iterations there are four more exponentiations of $\lambda$ bits (lines~\ref{line:batchpoeprove-regular} and~\ref{line:batchpoeprove-prime}). In total this amounts to $(1+\lfloor \log_2 q \rfloor) \cdot (b-1)  +  s \cdot (\lfloor \log_2 d \rfloor - \log_2(b) + 1) \cdot (1 + \lfloor \log_2 p \rfloor) + s \cdot \lambda \cdot (\lfloor \log_2 d \rfloor + \log_2 b) \cdot (\lfloor \log_2 d \rfloor - \log_2(b) + 1)/2  +  4 \cdot \lambda \cdot \lfloor \log_2 d \rfloor$ exponent bits, or $O_\lambda(\sqrt{d} \log d + s (\log d)^2)$ asymptotically.
...
By summing all terms, we have a total number of bits in exponents whose exact count is given below. Using the above approximations, this total number is asymptotically $O_\lambda(\sqrt{d} \cdot \log d + s \cdot (\log d)^2)$ \\
\begin{center}
\begin{tabular}{rrl} \\
...
line~\ref{line:evalprove-secondhalf-regular}: & $(\lfloor \log_2 d \rfloor - \log_2(b) + 1) \cdot (1 + \lfloor \log_2 p \rfloor + (\lfloor \log_2 d \rfloor + \log_2 b) \cdot \frac{\lambda}{2}) \cdot s$ \\
line~\ref{line:evalprove-secondhalf-prime}: & $(\lfloor \log_2 d \rfloor - \log_2(b) + 1) \cdot (1 + \lfloor \log_2 p \rfloor + (\lfloor \log_2 d \rfloor + \log_2 b) \cdot \frac{\lambda}{2}) \cdot s$ \\
line~\ref{line:evalprover-maintenance}: & $(b + \frac{b}{d}) \cdot \lambda$ \\
line~\ref{line:evalprove-batchpoe}: & $(1+\lfloor \log_2 q \rfloor) \cdot (b-1) $\\
 & $ +  s \cdot (\lfloor \log_2 d \rfloor - \log_2(b) + 1) \cdot (1 + \lfloor \log_2 p \rfloor) $ \\
 & $ + s \cdot \lambda \cdot (\lfloor \log_2 d \rfloor + \log_2 b) \cdot (\lfloor \log_2 d \rfloor - \log_2(b) + 1)/2 $ \\
 & $ +  4 \cdot \lambda \cdot \lfloor \log_2 d \rfloor$ \\ \hline
total: & $(2b + \log_2(b) - 1) \cdot (1 + \lfloor \log_2 q \rfloor)$ \\
 & $+ 3s \cdot (\lfloor \log_2 d \rfloor - \log_2(b) + 1) \cdot ( 1 + \lfloor \log_2 p \rfloor )$ \\
 & $ + \big(b + \frac{d}{b} + 4 \lfloor \log_2 d \rfloor \quad \quad$ \\
  & $ + \frac{s}{2} \cdot ( \lfloor \log_2 d \rfloor - \log_2(b) + 1) \cdot (\lfloor \log_2 d \rfloor + \log_2 b)\big) \cdot \lambda$ \\
 & $\in O_\lambda(\sqrt{d} \cdot \log d + s \cdot (\log d)^2)$ 
\end{tabular}
\end{center}
