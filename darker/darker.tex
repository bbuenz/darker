...
The shortest proof results from applying Wesolowski's batched $\mathsf{PoE}$~\cite{Wesolowski19}. However, it is possible to forego the Adaptive Root Assumption by opting instead for Pietrzak's $\mathsf{PoE}$\cite{Pietrzak19a}, in which case only one such $\mathsf{PoE}$ is needed.
