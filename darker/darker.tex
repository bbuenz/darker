...
\begin{enumerate}[nolistsep]
	...
	\item $\pi_{\mathsf{PoE}} \gets \mathsf{BatchPoE.Prove}((\mathsf{C}_O^{(0)}, \mathsf{C}_O^{\prime(0)}, \ldots, \mathsf{C}_O^{(\log_2(d)-1)}, \mathsf{C}_O^{\prime(\log_2(d)-1)})$ \label{line:evalprove-batchpoe}
...
\begin{enumerate}[nolistsep]
	...
	\item \pcind $\mathsf{Y}^{(i)} \gets q^{2^{i-1}} \cdot \mathsf{X}$ \label{line:batchpoeverify-q} $<---$ can't be right
