...
For the root node $\textsf{rt}$, define $\gr{X}_\textsf{rt} \deq \Commitment_\numrounds$ and $\gr{Z}_\textsf{rt} \deq \Commitment'_\numrounds$. For any non-leaf node $v$ on level $i$ with parent $w$ on level $i+1$, which has prover message $\gr{Y}_v$ and verifier challenge $\alpha_v$, define $\gr{X}_v \deq \gr{X}_w + \alpha_v \cdot \gr{Y}_w + \alpha^2 \cdot \Commitment_{i-1}$. Similarly, define $\gr{Z}_v \deq \gr{Y}_w + \alpha \cdot \gr{Z}_w + \alpha^2\cdot  \Commitment'_{i-1}$.
...
For the root node $\textsf{rt}$, define $\gr{C}_\textsf{rt} \deq \Commitment$. For any non-leaf node $v$ on level $i$ with parent $w$ on level $i+1$, which has prover message $\gr{C}_{E,v},\Commitment_{O,v}$ and verifier challenge $\alpha_v$, define $\gr{C}_v \deq \gr{C}_{E,w} + \alpha_v \cdot \gr{C}_{O,w}$.

We will show that given this tree, the extractor can compute for all nodes $v \in [0, d^{2})$ an opening hint $(\hat{f}_v \in \ZZ(b)[X],d_v \in \ZZ,\gr{W}_v \in \GG,\alpha_v \in \NN\setminus \{0\})$ for $\gr{C}_v$.   That is $\gr{C}_v^d=\Generator^{\hat{f}_v(q)}+\gr{W}_v$ and $\gr{W}_v^{\alpha_v}=0$ for $\alpha_v>0$. At level $i$, $||\hat{f}||_\infty \leq b\cdot 2^{(\log_2(d)+2(\logd-i)) \cdot \lambda}$
It will do this by starting at the leaves and working its way up the tree.
For any leaf node $v$, the fact that the transcript is accepting implies $\Commitment_v=f\cdot \Generator$ for $f\leq b \cdot 2^{\logd \lambda}$. $f$ can be decoded to a constant polynomial with a coefficient bounded by $b \cdot 2^{\logd \lambda}$. $(f,1,0,1)$ is therefore a valid opening for $\Commitment_v$.
 Now suppose that the appropriate openings have been computed for all children of a node $w$ on the $i$th level. Denoting the two children of nodes $w$ with indices $v_1, v_2$ and the two challenges on each by $\alpha_1, \alpha_2$ \benedikt{use $\alpha$, $\alpha$'}. This means that $\gr{C}_{v_1} = \gr{C}_{E,w} + \alpha_1 \cdot \gr{C}_{O,w}$ and $\gr{C}_{v_2} = \gr{C}_{E,w} + \alpha_2 \cdot \gr{C}_{O,w}$.
Furthermore openings $(\hat{f}_{v_1},d_{v_1},\gr{W}_{v_1},\alpha_{v_1})$ for $\gr{C}_{v_1}$ and  $(\hat{f}_{v_2},d_{v_2},\gr{W}_{v_2},\alpha_{v_2})$ for $\gr{C}_{v_2}$ have been computed. 
\benedikt{Fix challenge/order overflow}
