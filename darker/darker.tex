...
The protocols and algorithms are presented below. We use additive notation for the groups and denote the identity element by 0. In argument lists and return values, we use the semicolon to distinguish public values (on the left) from secret ones (on the right). All algorithms have access to the (implicit) public parameters $(\mathbb{G}, \mathsf{G}, d, b)$ and without loss of generality the degree $d$ of the polynomial $f(X)$ is one less than a power of two. The function $\mathsf{lift} : \mathbb{F}_p[X] \rightarrow \mathbb{Z}[X]$ lifts a polynomial defined over a field to its canonical representative over the integers.
...
\begin{enumerate}[nolistsep]
	...
	\item \pcind \pcind $\mathsf{P}_i \gets \mathsf{P}_i + \hat{h_j} \cdot \mathsf{G}_{\lfloor j / b \rfloor}$ 
	\item $\mathsf{C} \gets \mathsf{P}_{b-1}$
	\item \textbf{for} $i$ \textbf{from} $b-2$ \textbf{down to} $0$ \textbf{:}
	\item \pcind $\mathsf{C} \gets \mathsf{P}_{i} + q \cdot \mathsf{C}$
...
\begin{enumerate}[nolistsep]
	\item $\pi \gets \varnothing$
	\item \textbf{for} $i$ \textbf{from} $0$ \textbf{to} $\log_2(d) - 1$ \textbf{:}
	\item \pcind $\hat{f}_E \gets \frac{\hat{f}(X) + \hat{f}(-X)}{2}$ and $\hat{f}_O \gets \hat{f}(X) - \hat{f}_E(X)$
	\item \pcind $\mathbf{y}_O^{(i)} \gets \hat{f}_O(\mathbf{z})$
	...
	\item \pcind \pcind \pcind $\mathsf{C}_O^{(i)} \gets \mathsf{P}_{b-1-2j} + q^{2^i} \cdot \mathsf{C}_O^{(i)}$
	\item \pcind \pcind $\mathsf{C}_O^{\prime(i)} \gets q \cdot \mathsf{C}_O^{(i)}$
	\item \pcind \textbf{else:}
	\item \pcind \pcind $\mathsf{C}_O^{(i)} \gets 0$ and $\mathsf{C}_O^{\prime(i)} \gets 0$
	\item \pcind \pcind \textbf{for all} nonzero terms $\hat{h}_jX^j$ of $\hat{f}_O(X^{2^i})$ \textbf{:}
	\item \pcind \pcind \pcind $\mathsf{C}_O^{(i)} \gets \mathsf{C}_O^{(i)} + \hat{h}_j \cdot \mathsf{G}_{\lfloor j/b \rfloor}$
	\item \pcind \pcind \pcind $\mathsf{C}_O^{\prime(i)} \gets \mathsf{C}_O^{\prime(i)} + \hat{h}_j \cdot \mathsf{G}_{\lfloor (j+2^i)/b\rfloor}$
	\item \pcind $\pi \gets \pi \Vert \mathbf{y}_O^{(i)} \Vert \mathsf{C}_O^{(i)} \Vert \mathsf{C}_O^{\prime(i)}$
	\item \pcind $\alpha \gets \mathsf{H}(\mathsf{C} \Vert \mathbf{z} \Vert \mathbf{y} \Vert \pi)$  \pccomment{Fiat-Shamir}
	\item \pcind $\hat{f} \gets \hat{f}_E(X) + \alpha \cdot \hat{f}_O(X)$
	\item \pcind \textbf{for} $j$ \textbf{from} $0$ \textbf{to} $b/2^{i+1}-1$ \textbf{:}
	\item \pcind \pcind $\mathsf{P}_j \gets \mathsf{P}_j + \alpha \cdot \mathsf{P}_{b/2^{i+1}+j}$
	\item $\pi_{\mathsf{PoE}} \gets \mathsf{BatchPoE.Prove}((\mathsf{C}_O^{(0)}, \mathsf{C}_O^{\prime(0)}, \ldots, \mathsf{C}_O^{(\log_2(d)-1)}, \mathsf{C}_O^{\prime(\log_2(d)-1)})$
	\item \textbf{return} $\pi \Vert \pi_{\mathsf{PoE}} \Vert \hat{f}$
\end{enumerate}
$\mathsf{Eval.Verify}(\mathsf{C}, \mathbf{z}, \mathbf{y}, \pi)$ \textbf{:}
\begin{enumerate}[nolistsep]
	\item $\mathsf{C}_O^{(0)} \Vert \mathsf{C}_O^{\prime(0)} \Vert \mathbf{y}_O^{(0)} \Vert \cdots \Vert \mathsf{C}_O^{(\log_2(d) - 1)} \Vert \mathsf{C}_O^{\prime(\log_2(d)-1)} \Vert \mathbf{y}_O^{(\log_2 (d) - 1)} \Vert \pi_{\mathsf{PoE}} \Vert \hat{f} \gets \pi$
	\item $\tau \gets \mathsf{C} \Vert \mathbf{z} \Vert \mathbf{y}$
	\item \textbf{for} $i$ \textbf{from} $0$ \textbf{to} $\log_2(d)-1$ \textbf{:}
	\item \pcind $\tau \gets \tau \Vert \mathsf{C}_O^{(i)} \Vert \mathsf{C}_O^{\prime(i)} \Vert \mathbf{y}_O^{(i)}$
	\item \pcind $\alpha \gets \mathsf{H}(\tau)$
	\item \pcind $\mathsf{C} \gets \mathsf{C} - \mathsf{C}_O^{\prime(i)} + \alpha \cdot \mathsf{C}_O^{(i)}$
	\item \pcind $\mathbf{y} \gets \mathbf{y} - \mathbf{z} \circ \mathbf{y}_O^{(i)} + \alpha \cdot \mathbf{y}_O^{(i)} \mod p$
	\item $b_{\mathsf{PoE}} \gets \mathsf{BatchPoE.Verify}((\mathsf{C}_O^{(0)}, \mathsf{C}_O^{\prime(0)}, \ldots, \mathsf{C}_O^{(\log_2(d)-1)}, \mathsf{C}_O^{\prime(\log_2(d)-1)}))$
...
\begin{mdframed}
$\mathsf{BatchPoE.Prove}((\mathsf{C}_O^{(0)}, \mathsf{C}_O^{\prime(0)}, \ldots, \mathsf{C}_O^{(\log_2(d)-1)}, \mathsf{C}_O^{\prime(\log_2(d)-1)}))$ \textbf{:}
\begin{enumerate}[nolistsep]
	\item $\pi \gets \varnothing$
	...
	\item \pcind $\mathsf{Y}^{(i)} \gets q^{2^{i-1}} \cdot \mathsf{X}$
