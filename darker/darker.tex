...
\begin{proof}
The least common multiple of all denominators of $f_E(X) + \alpha f_O(X)$ being less than $\frac{B}{2^\lambda}$ implies the same for all individual denominators. Let $f_{E,i}$ and $f_{O,i}$ to denote the $i$th coefficient of their respective polynomials. The probability in question is therefore less than
\begin{equation}
...
\end{equation}
for the $i$ that minimizes this probability.
...
Note that $D'$, $d_O' f^\star_{E,i}$, and $d_E' f^\star_{O,i}$ are mutually co-prime. Reshuffling gives
$$ \mathsf{gcd}(D', d_O' f^\star_{E,i} + \alpha d_E' f^\star_{O,i}) > \frac{2^\lambda D'}{B} \enspace ,$$
